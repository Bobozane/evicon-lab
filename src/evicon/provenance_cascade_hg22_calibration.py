"""Isolated H-G.2.2 real eligibility-calibration runner and offline gates."""
...
from .provenance_cascade_hg22_compatibility_receipt import validate_receipt
...
DEFAULT_EXECUTION_APPROVAL = "configs/provenance_cascade/pilot/provenance_cascade_pilot_hg22_execution_approval.toml"
...
class RecordingHG22Runtime(HG22Runtime):
...
class HG22CalibrationRunner:
    def __init__(self, config_path: str | Path = DEFAULT_CONFIG) -> None:
        ...
        self.runs = tuple(
            HG21RunSpec(
                run_id=f"hg22-{scenario_id}-{self.config_lock.seed}-{condition.value}",
                matched_group_id=f"hg22-{scenario_id}-{self.config_lock.seed}",
    ...
    def run_one(
        ...
        *,
        ...
        model_name: str,
        resume: bool = False,
    ) -> tuple[CascadeRealAgentRunRecord, tuple[HG21DecisionRecord, ...]]:
        ...
        runtime = RecordingHG22Runtime(decision_path, binding)
    ...
    def run_all(
        ...
        *,
        ...
        design = HG22Approval.model_validate(tomllib.loads(_path(DEFAULT_APPROVAL).read_text(encoding="utf-8")))
...
def final_preflight() -> dict[str, object]:
    ...
    try:
        ...
        if compatibility.model != config.required_model_name:
        ...
        return {
            ...
            "blocking_reasons": [getattr(exc, "code", "hg22_calibration_preflight_failed")],
            "network": "disabled",
            "provider_constructed": False,
            "api_key_read": False,
            "results_written": False,
...
def run_fake_smoke() -> dict[str, object]:
    runner = HG22CalibrationRunner()
    with tempfile.TemporaryDirectory(prefix="evicon-hg22-calibration-") as temporary:
        root = Path(temporary) / "calibration"
        records, receipt = runner.run_all(
            provider_factory=lambda _spec: HG2FakeProvider(),
            root=root,
            model_name=runner.config_lock.required_model_name,
            resume=False,
            write_receipt=False,
            network="disabled",
        )
...
def execute_real(
    *,
    ...
    runner, constructed = HG22CalibrationRunner(), 0
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="H-G.2.2 eligibility calibration")
