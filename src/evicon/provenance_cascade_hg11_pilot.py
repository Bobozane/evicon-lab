"""Isolated, opt-in H-G.1.1 Pilot runner with offline-first gates."""
...
from .cascade_agent_protocol_hg11 import (
    HG11_PROTOCOL_VERSION,
    HG11_SCHEMA_NAME,
    HG11_TEMPLATE_VERSION,
    CascadeAgentProtocolHG11Runtime,
...
from .cascade_hg1_replay import HG1_OUTCOME_REPLAY_VERSION, HG1CascadeProtocolRunner, HG1OutcomeReplayValidator
...
from .provenance_cascade_hg11 import (
    HG11Approval,
    HG11FakeProvider,
    HG11PilotConfig,
    HG11RunSpec,
...
from .provenance_cascade_hg11_compatibility_receipt import validate_receipt
from .request_ledger import RequestLedger

_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_CONFIG_RELATIVE = "configs/provenance_cascade/pilot/provenance_cascade_pilot_hg11.v1.toml"
DEFAULT_APPROVAL_RELATIVE = "configs/provenance_cascade/pilot/provenance_cascade_pilot_hg11_approval_template.toml"
DEFAULT_PROTOCOL_RELATIVE = "src/evicon/cascade_agent_protocol_hg11.py"
DEFAULT_COMPATIBILITY_RECEIPT_RELATIVE = "outputs/study-locks/provenance_cascade_hg11_compatibility_receipt.json"
DEFAULT_AMENDMENT_RECEIPT_RELATIVE = "outputs/study-locks/provenance_cascade_hg11_truncation_amendment_receipt.json"
DEFAULT_OUTPUT_ROOT = "results/provenance-cascade-pilot-hg11-v1"


class HG11PilotRunStatus(str, Enum):
    PLANNED = "planned"
    RUNNING = "running"
    COMPLETED = "completed"
    FAILED = "failed"
...
class HG11PilotRunState(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    run_id: str
    scenario_id: str
    seed: int
    condition: str
    status: HG11PilotRunStatus = HG11PilotRunStatus.PLANNED
...
class HG11PilotBatchRecord(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    batch_version: str = "provenance_cascade_hg11_execution.v1"
    outcome_replay_contract_version: str = HG1_OUTCOME_REPLAY_VERSION
    study_id: str
    status: HG11PilotRunStatus
    ...
    controller_sha256: str = Field(min_length=64, max_length=64)
...
class HG11PilotReceipt(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    status: str
    study_id: str
    config_sha256: str
...
class HG11PilotExecutionSummary(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    status: str
    error_code: str | None = None
...
def _load_approval(path: Path) -> HG11Approval:
    import tomllib
    try:
        return HG11Approval.model_validate(tomllib.loads(path.read_text(encoding="utf-8")))
    except Exception as exc:
        raise CascadeRealAgentRunError("approval_invalid") from exc


class HG11PilotRunner:
    """Execute registered H-G.1.1 coordinates without parser recovery."""

    def __init__(self, config_path: str | Path = DEFAULT_CONFIG_RELATIVE) -> None:
        self.config_path = _rooted(config_path)
        self.config, self.scenarios, self.truths, self.schedules = load_hg11_config(self.config_path)
        self.policy = CascadeControllerPolicyLoader.load(_rooted(self.config.policy_path))
        self.protocol_runner = HG1CascadeProtocolRunner()
        self.base_runner = CascadeRealAgentRunner(
            base_runner=self.protocol_runner, policy_config=self.policy
        )
    ...
    def run_one(
        ...
        *,
        ...
        return self.base_runner.run_scenario(
            ...
            claim_selector=lambda scenario, snapshot, round_id: _claim_selector(
                schedule, scenario, snapshot, round_id
            ),
            proposal_builder=propose_hg1,
            proposal_validator=validate_hg1_proposal,
            post_round_events_builder=lambda scenario, round_id, events: _post_round_events(
                scenario, schedule, round_id, events
            ),
            actor_schedule_provider=lambda _scenario, round_id: (
    ...
    def run_all(
        ...
        *,
        ...
        network: str = "enabled",
    ) -> tuple[list[CascadeRealAgentRunRecord], HG11PilotReceipt]:
        ...
        for spec in self.config.runs:
            ...
            if record_path.exists():
                ...
                batch = _replace_state(batch, _state_from_record(spec, record, run_dir), HG11PilotRunStatus.RUNNING, None)
                _atomic_model(batch_path, batch)
                continue
            if state.status is HG11PilotRunStatus.COMPLETED:
            ...
            batch = _replace_state(
                batch,
                state.model_copy(update={"status": HG11PilotRunStatus.RUNNING, "error_code": None}),
                HG11PilotRunStatus.RUNNING,
                None,
            )
            _atomic_model(batch_path, batch)
            try:
                ...
                failed = state.model_copy(update={"status": HG11PilotRunStatus.FAILED, "error_code": "provider_construction_failed"})
                batch = _replace_state(batch, failed, HG11PilotRunStatus.FAILED, "provider_construction_failed")
                _atomic_model(batch_path, batch)
            ...
            try:
                ...
                failed = state.model_copy(update={"status": HG11PilotRunStatus.FAILED, "error_code": exc.code})
                batch = _replace_state(batch, failed, HG11PilotRunStatus.FAILED, exc.code)
                _atomic_model(batch_path, batch)
                raise
            ...
            batch = _replace_state(batch, _state_from_record(spec, safe_record, run_dir), HG11PilotRunStatus.RUNNING, None)
            _atomic_model(batch_path, batch)
        replay_passed = sum(record.replay is not None and record.replay.status.value == "passed" for record in records)
        if len(records) != 48 or replay_passed != 48 or any(item.status is not HG11PilotRunStatus.COMPLETED for item in batch.runs):
            raise CascadeRealAgentRunError("batch_replay_incomplete")
        aggregates = _aggregate_ledgers(root_path, self.config)
        receipt = HG11PilotReceipt(
            status="completed",
        ...
        if write_receipt:
            if receipt_path.exists():
                ...
                existing = HG11PilotReceipt.model_validate_json(receipt_path.read_text(encoding="utf-8"))
        ...
        batch = batch.model_copy(update={"status": HG11PilotRunStatus.COMPLETED, "failure_code": None})
        _atomic_model(batch_path, batch)
...
def _initial_batch(
    ...
    *,
    ...
    compatibility_receipt_sha256: str,
    amendment_sha256: str,
) -> HG11PilotBatchRecord:
    return HG11PilotBatchRecord(
        study_id=config.study_id,
        status=HG11PilotRunStatus.RUNNING,
...
def _load_batch(path: Path) -> HG11PilotBatchRecord:
    if not path.is_file():
        raise CascadeRealAgentRunError("resume_batch_record_missing")
    try:
        return HG11PilotBatchRecord.model_validate_json(path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise CascadeRealAgentRunError("batch_record_invalid") from exc


def _validate_batch(batch: HG11PilotBatchRecord, config: HG11PilotConfig, config_path: Path, binding: str, root: Path, model: str) -> None:
    ...
    if batch.controller_sha256 != config.controller_sha256:
        raise CascadeRealAgentRunError("resume_controller_hash_mismatch")
    if batch.replay_sha256 != config.replay_sha256:
        raise CascadeRealAgentRunError("resume_replay_hash_mismatch")


def _replace_state(batch: HG11PilotBatchRecord, state: HG11PilotRunState, status: HG11PilotRunStatus, code: str | None) -> HG11PilotBatchRecord:
...
def _load_run_record(path: Path, spec: HG11RunSpec, scenario: Any) -> CascadeRealAgentRunRecord:
    try:
...
def _state_from_record(spec: HG11RunSpec, record: CascadeRealAgentRunRecord, run_dir: Path) -> HG11PilotRunState:
    ...
    if replay is None:
        raise CascadeRealAgentRunError("run_replay_incomplete")
    return HG11PilotRunState(
        run_id=spec.run_id,
        scenario_id=spec.scenario_id,
        seed=spec.seed,
        condition=spec.condition.value,
        status=HG11PilotRunStatus.COMPLETED,
...
def _aggregate_ledgers(root: Path, config: HG11PilotConfig) -> dict[str, int | None]:
...
def run_hg11_pilot_fake_smoke(config_path: str | Path = DEFAULT_CONFIG_RELATIVE) -> dict[str, object]:
    runner = HG11PilotRunner(config_path)
    with tempfile.TemporaryDirectory(prefix="evicon-hg11-pilot-fake-") as temporary:
        records, receipt = runner.run_all(
            provider_factory=lambda _spec: HG11FakeProvider(),
            root=Path(temporary) / "pilot",
            model_name="hg11-truncation-fake",
...
def _blocked(code: str) -> HG11PilotExecutionSummary:
    return HG11PilotExecutionSummary(
        status="blocked",
        error_code=code,
        network="disabled",
...
def _partial_failure_summary(
    *,
    ...
    try:
        ...
        completed_run_count = sum(item.status is HG11PilotRunStatus.COMPLETED for item in batch.runs)
        failed_run_count = sum(item.status is HG11PilotRunStatus.FAILED for item in batch.runs)
        ...
        replay_statuses = {
            ...
            if item.status in {HG11PilotRunStatus.COMPLETED, HG11PilotRunStatus.FAILED}
        }
...
def final_preflight_hg11(
    config_path: str | Path = DEFAULT_CONFIG_RELATIVE,
    approval_path: str | Path = DEFAULT_APPROVAL_RELATIVE,
    *,
    resume: bool = False,
) -> dict[str, object]:
    """Validate all locked H-G.1.1 inputs without reading environment or constructing a Provider."""
    reasons: list[str] = []
    try:
        config, scenarios, _, _ = load_hg11_config(config_path)
    except Exception as exc:
        return {
            "status": "blocked",
            "ready_for_real_pilot": False,
            "blocking_reasons": [getattr(exc, "code", "config_invalid")],
            "network": "disabled",
    ...
    approval: HG11Approval | None = None
    if not approval_file.is_file():
        ...
        try:
            ...
            expected = (
                sha256_file(config_file), config.protocol_sha256,
                config.controller_sha256, config.replay_sha256,
                config.amendment_sha256,
            )
            actual = (
                approval.config_sha256, approval.protocol_sha256,
                approval.controller_sha256, approval.replay_sha256,
                approval.amendment_sha256,
            )
            if expected != actual:
                reasons.append("approval_hash_mismatch")
        except Exception:
            reasons.append("approval_invalid")
    amendment_receipt_file = _rooted(DEFAULT_AMENDMENT_RECEIPT_RELATIVE)
    if not amendment_receipt_file.is_file():
        reasons.append("amendment_receipt_missing")
    elif approval is None or sha256_file(amendment_receipt_file) != approval.amendment_receipt_sha256:
        reasons.append("amendment_receipt_hash_mismatch")
    else:
        try:
            amendment_receipt = json.loads(amendment_receipt_file.read_text(encoding="utf-8"))
            if (
                amendment_receipt.get("config_sha256") != sha256_file(config_file)
                or amendment_receipt.get("protocol_sha256") != config.protocol_sha256
                or amendment_receipt.get("controller_sha256") != config.controller_sha256
                or amendment_receipt.get("replay_sha256") != config.replay_sha256
                or amendment_receipt.get("completion_reservation_cap") != config.completion_reservation_cap
                or amendment_receipt.get("fake_provider_replay_passed_count") != 48
            ):
                reasons.append("amendment_receipt_invalid")
        except Exception:
            reasons.append("amendment_receipt_invalid")
    compatibility_file = _rooted(config.compatibility_receipt_path)
    if approval is None or not approval.compatibility_receipt_sha256:
        ...
        try:
            ...
            reasons.append(getattr(exc, "code", "compatibility_receipt_invalid"))
    output_root = _rooted(config.output_root)
    if output_root.exists() and not resume:
        reasons.append("output_root_exists")
    if resume and not output_root.exists():
        reasons.append("resume_output_missing")
    return {
        ...
        "controller_sha256": config.controller_sha256,
        "replay_sha256": config.replay_sha256,
        "approval_sha256": sha256_file(approval_file) if approval_file.is_file() else None,
        "compatibility_receipt_sha256": (
            sha256_file(compatibility_file) if compatibility_file.is_file() else None
        ),
        "scenario_count": len(scenarios), "condition_count": 4,
        "seed_count": 3, "run_count": len(config.runs),
        ...
        "output_root": config.output_root, "resume": resume,
...
def execute_hg11_real_pilot(
    *,
    ...
    if confirm_completion_reservation_cap != 884736:
        return _blocked("confirm_completion_reservation_cap_must_equal_884736")
    gate = final_preflight_hg11(config_path, approval_path, resume=resume)
    ...
    runner = HG11PilotRunner(config_path_resolved)
    ...
    def provider_factory(spec: HG11RunSpec) -> LLMProvider:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="H-G.1.1 public-content-identifiable provenance-cascade Pilot")
    ...
    if args.mode == "preflight":
        payload = final_preflight_hg11(args.config, args.approval, resume=args.resume)
    elif args.mode == "fake-smoke":
        payload = run_hg11_pilot_fake_smoke(args.config)
    else:
        payload = execute_hg11_real_pilot(
