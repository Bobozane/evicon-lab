"""Isolated H-G.2.3.1 real eligibility-calibration runner and offline gates."""
...
from .cascade_agent_protocol_hg231 import (
    HG231Runtime,
    HG231_PROTOCOL_VERSION,
    HG231_RESPONSE_JSON_SCHEMA,
    HG231_SCHEMA_NAME,
    HG231_TEMPLATE_VERSION,
)
...
from .provenance_cascade_hg231_compatibility_receipt import validate_receipt
...
DEFAULT_EXECUTION_APPROVAL = "configs/provenance_cascade/pilot/provenance_cascade_pilot_hg231_execution_approval.toml"
...
class HG231DecisionRecord(BaseModel):
...
def load_hg231_decisions(path: Path, binding: str) -> tuple[HG231DecisionRecord, ...]:
    if not path.exists():
        return ()
    records: list[HG231DecisionRecord] = []
    fingerprints: set[str] = set()
    try:
        for line in path.read_text(encoding="utf-8").splitlines():
            if not line.strip():
                continue
            item = HG231DecisionRecord.model_validate_json(line)
...
class RecordingHG231Runtime(HG231Runtime):
    ...
    def execute(self, context, provider, *, request_metadata: Mapping[str, Any] | None = None) -> HG2RuntimeResult:
        ...
        existing = {entry.fingerprint: entry for entry in load_hg231_decisions(self.path, self.binding_sha256)}
        record = HG231DecisionRecord(
...
class HG231CalibrationRunner:
    def __init__(self, config_path: str | Path = DEFAULT_CONFIG) -> None:
        ...
        self.runs = tuple(
            HG21RunSpec(
                run_id=f"hg231-{scenario_id}-{self.config_lock.seed}-{condition.value}",
                matched_group_id=f"hg231-{scenario_id}-{self.config_lock.seed}",
    ...
    def run_one(
        ...
        *,
        ...
        model_name: str,
        resume: bool = False,
    ) -> tuple[CascadeRealAgentRunRecord, tuple[HG231DecisionRecord, ...]]:
        ...
        runtime = RecordingHG231Runtime(decision_path, binding)
        ...
        decisions = load_hg231_decisions(decision_path, binding)
    ...
    def run_all(
        ...
        *,
        ...
        design = HG231Approval.model_validate(tomllib.loads(_path(DEFAULT_APPROVAL).read_text(encoding="utf-8")))
        ...
        all_decisions: list[HG231DecisionRecord] = []
        for spec in self.runs:
            ...
            if record_path.exists():
                ...
                decisions = load_hg231_decisions(run_dir / "behavior_decisions.jsonl", self._run_binding(spec, model_name))
                if len(decisions) != 18:
                    raise CascadeRealAgentRunError("behavior_decision_checkpoint_incomplete")
...
def final_preflight() -> dict[str, object]:
    ...
    try:
        ...
        return {
            ...
            "blocking_reasons": [getattr(exc, "code", "hg231_calibration_preflight_failed")],
            "network": "disabled",
            "provider_constructed": False,
            "api_key_read": False,
            "results_written": False,
...
def run_fake_smoke() -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="evicon-hg231-calibration-") as temporary:
...
def execute_real(
    *,
    ...
    runner, constructed = HG231CalibrationRunner(), 0
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="H-G.2.3.1 eligibility calibration")
