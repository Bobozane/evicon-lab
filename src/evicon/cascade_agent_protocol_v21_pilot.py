"""Isolated H-E pilot boundary for the H-D.2.1 strict Agent protocol.
...
H-D.2.1 contract and never enables parser recovery.
"""
...
from .cascade_agent_protocol_v21_compatibility import HD21FakeProvider
from .cascade_agent_protocol_v21_compatibility import load_compatibility_receipt
...
HD21_TEMPLATE_RELATIVE = HD21_PROTOCOL_RELATIVE
HD21_OUTPUT_ROOT = "results/provenance-cascade-pilot-hd21-v1"
COMPATIBILITY_RECEIPT_RELATIVE = "outputs/study-locks/provenance_cascade_agent_v21_compatibility_receipt.json"
APPROVAL_VERSION = "provenance_cascade_hd21_researcher_approval.v1"
...
class HD21Approval(BaseModel):
    ...
    def accepted_fields(self) -> "HD21Approval":
...
class HD21PilotReceipt(BaseModel):
...
class HD21PilotRunStatus(str, Enum):
    PLANNED = "planned"
    RUNNING = "running"
    COMPLETED = "completed"
    FAILED = "failed"
...
class HD21PilotRunState(BaseModel):
    ...
    seed: int
    condition: str
    status: HD21PilotRunStatus = HD21PilotRunStatus.PLANNED
...
class HD21PilotBatchRecord(BaseModel):
    """Safe resumable batch state; request ledgers remain append-only."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    batch_version: str = "provenance_cascade_hd21_execution.v1"
    study_id: str
    status: HD21PilotRunStatus
...
class HD21PilotExecutionSummary(BaseModel):
    """Safe CLI result for completed, failed, or blocked H-D.2.1 execution."""
...
def load_approval(path: str | Path) -> HD21Approval:
    try:
        payload = tomllib.loads(Path(path).read_text(encoding="utf-8"))
        return HD21Approval.model_validate(payload)
    except (OSError, tomllib.TOMLDecodeError, ValidationError, ValueError) as exc:
        raise ValueError("approval_invalid") from exc


def _load_scenarios(config_path: Path, config: HD21Config) -> dict[str, Any]:
    hd2_path = (config_path.parent / config.parent_config_path).resolve()
    try:
        hd2_payload = tomllib.loads(hd2_path.read_text(encoding="utf-8"))
        parent_path = (hd2_path.parent / str(hd2_payload["parent_config_path"])).resolve()
    except (OSError, tomllib.TOMLDecodeError, KeyError, TypeError) as exc:
        raise HD21ConfigError("parent_scenario_binding_invalid") from exc
...
def _safe_compatibility_ok(path: Path, expected_hash: str | None) -> tuple[bool, str]:
    """Validate only the safe, bound H-D.2.1 compatibility receipt fields."""
    try:
        if expected_hash is None or sha256_file(path) != expected_hash.lower():
            return False, "compatibility_receipt_hash_mismatch"
        receipt = load_compatibility_receipt(path)
        if receipt.config_sha256 != sha256_file(_ROOT / HD21_CONFIG_RELATIVE):
            return False, "compatibility_config_hash_mismatch"
        if receipt.protocol_sha256 != sha256_file(_ROOT / HD21_PROTOCOL_RELATIVE):
            return False, "compatibility_protocol_hash_mismatch"
        if receipt.template_sha256 != sha256_file(_ROOT / HD21_TEMPLATE_RELATIVE):
            return False, "compatibility_template_hash_mismatch"
        if receipt.response_format != "json_schema" or receipt.schema_name != V21_SCHEMA_NAME:
            return False, "compatibility_response_contract_mismatch"
        if receipt.max_tokens != 512 or receipt.temperature != 0.2 or receipt.seed != 20260911:
            return False, "compatibility_generation_parameters_mismatch"
        if receipt.max_retries != 0:
            return False, "compatibility_retry_scope_mismatch"
        if not receipt.completed:
            return False, "compatibility_network_check_pending"
        return True, ""
    except (OSError, ValueError):
        return False, "compatibility_receipt_invalid"
...
def final_preflight(config_path: str | Path, approval_path: str | Path, *, resume: bool = False) -> dict[str, object]:
    ...
    reasons: list[str] = []
    if config_file != (_ROOT / HD21_CONFIG_RELATIVE).resolve():
        reasons.append("config_path_not_hd21")
    try:
        config, _ = load_hd21_config(config_file)
    except HD21ConfigError as exc:
        result["blocking_reasons"] = [exc.code]
        return result
    result.update({
        "study_id": config.study_id,
        "config_sha256": sha256_file(config_file),
        "protocol_sha256": sha256_file(_ROOT / HD21_PROTOCOL_RELATIVE),
        "template_sha256": sha256_file(_ROOT / HD21_TEMPLATE_RELATIVE),
    ...
    expected_protocol_hash = sha256_file(_ROOT / HD21_PROTOCOL_RELATIVE)
    if approval.config_path != HD21_CONFIG_RELATIVE or approval.config_sha256.lower() != expected_config_hash:
        reasons.append("config_hash_mismatch")
    if approval.protocol_path != HD21_PROTOCOL_RELATIVE or approval.protocol_sha256.lower() != expected_protocol_hash:
        reasons.append("protocol_hash_mismatch")
    expected_template_hash = sha256_file(_ROOT / HD21_TEMPLATE_RELATIVE)
    if approval.template_path != HD21_TEMPLATE_RELATIVE or approval.template_sha256.lower() != expected_template_hash:
        reasons.append("template_hash_mismatch")
    if config.response_format != "json_schema" or config.response_schema_name != V21_SCHEMA_NAME:
        reasons.append("response_format_mismatch")
    if config.output_root != HD21_OUTPUT_ROOT or "002" in config.output_root or "003" in config.output_root or "calibration" in config.output_root:
    ...
    if approval.compatibility_receipt_path:
        ...
        if not compatible:
            ...
            result["compatibility_module_version"] = "cascade_agent_protocol_v21_compatibility.v1"
            result["compatibility_max_tokens"] = 512
    ...
    if config.request_cap != 864 or config.completion_reservation_cap != 442368:
...
class HD21PilotRunner:
    """Run one H-D.2.1 coordinate with the strict runtime and no parser recovery."""

    def __init__(self, config_path: str | Path):
        self.config_path = Path(config_path).resolve()
        self.config, _ = load_hd21_config(self.config_path)
    ...
    def run_one(
        ...
        *,
        ...
        return self.base_runner.run_scenario(
            ...
            model_name=model_name, agent_temperature=0.2, agent_max_tokens=512,
            ...
            runtime=CascadeAgentProtocolV21Runtime(),
        )
    ...
    def run_all(
        ...
        *,
        ...
        compatibility_receipt_sha256: str | None = None,
        network: str = "enabled",
    ) -> tuple[list[CascadeRealAgentRunRecord], HD21PilotReceipt]:
        ...
        expected_root = (_ROOT / HD21_OUTPUT_ROOT).resolve()
        ...
        for spec in self.config.runs:
            ...
            if run_record_path.exists():
                if not resume and state.status is not HD21PilotRunStatus.COMPLETED:
                ...
                batch = _replace_run_state(batch, updated, status=HD21PilotRunStatus.RUNNING, failure_code=None)
                _atomic_model_json(batch_path, batch)
                continue
            if state.status is HD21PilotRunStatus.COMPLETED:
                raise CascadeRealAgentRunError("completed_run_record_missing")
            if run_dir.exists() and not resume and any(run_dir.iterdir()):
                raise CascadeRealAgentRunError("run_output_exists")
            running = state.model_copy(update={"status": HD21PilotRunStatus.RUNNING, "error_code": None})
            batch = _replace_run_state(batch, running, status=HD21PilotRunStatus.RUNNING, failure_code=None)
            _atomic_model_json(batch_path, batch)
            try:
                provider = provider_factory(spec)
            except Exception:
                failed = state.model_copy(update={"status": HD21PilotRunStatus.FAILED, "error_code": "provider_construction_failed"})
                batch = _replace_run_state(batch, failed, status=HD21PilotRunStatus.FAILED, failure_code="provider_construction_failed")
                _atomic_model_json(batch_path, batch)
            ...
            try:
                ...
                failed = state.model_copy(update={"status": HD21PilotRunStatus.FAILED, "error_code": exc.code})
                batch = _replace_run_state(batch, failed, status=HD21PilotRunStatus.FAILED, failure_code=exc.code)
                _atomic_model_json(batch_path, batch)
                raise
            except Exception:
                failed = state.model_copy(update={"status": HD21PilotRunStatus.FAILED, "error_code": "pilot_execution_failed"})
                batch = _replace_run_state(batch, failed, status=HD21PilotRunStatus.FAILED, failure_code="pilot_execution_failed")
                _atomic_model_json(batch_path, batch)
            ...
            batch = _replace_run_state(batch, completed, status=HD21PilotRunStatus.RUNNING, failure_code=None)
            _atomic_model_json(batch_path, batch)

        replay_passed = sum(record.replay is not None and record.replay.status.value == "passed" for record in records)
        if len(records) != 48 or replay_passed != 48 or any(item.status is not HD21PilotRunStatus.COMPLETED for item in batch.runs):
        ...
        if write_receipt:
            if receipt_path.exists():
                ...
                try:
                    existing = HD21PilotReceipt.model_validate_json(receipt_path.read_text(encoding="utf-8"))
        ...
        batch = batch.model_copy(update={"status": HD21PilotRunStatus.COMPLETED, "failure_code": None})
        _atomic_model_json(batch_path, batch)
...
def _initial_batch_record(
    ...
    *,
    ...
    return HD21PilotBatchRecord(
        study_id=config.study_id,
        status=HD21PilotRunStatus.RUNNING,
...
def _load_batch_record(path: Path) -> HD21PilotBatchRecord:
    if not path.exists():
        raise CascadeRealAgentRunError("resume_batch_record_missing")
    try:
        return HD21PilotBatchRecord.model_validate_json(path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise CascadeRealAgentRunError("batch_record_invalid") from exc


def _validate_batch_binding(
    batch: HD21PilotBatchRecord,
    config: HD21Config,
    *,
    binding: str,
    model_name: str,
    ...
    if batch.config_sha256 != sha256_file(_ROOT / HD21_CONFIG_RELATIVE):
        raise CascadeRealAgentRunError("resume_config_hash_mismatch")
    if batch.protocol_sha256 != sha256_file(_ROOT / HD21_PROTOCOL_RELATIVE) or batch.template_sha256 != sha256_file(_ROOT / HD21_TEMPLATE_RELATIVE):
        raise CascadeRealAgentRunError("resume_protocol_hash_mismatch")


def _replace_run_state(
    batch: HD21PilotBatchRecord,
    state: HD21PilotRunState,
    *,
    status: HD21PilotRunStatus,
...
def _load_completed_run_record(path: Path, spec: HD21RunSpec) -> CascadeRealAgentRunRecord:
    try:
...
def _state_from_record(spec: HD21RunSpec, record: CascadeRealAgentRunRecord, run_dir: Path) -> HD21PilotRunState:
    ...
    if replay is None:
        raise CascadeRealAgentRunError("run_replay_incomplete")
    return HD21PilotRunState(
        run_id=spec.run_id,
        scenario_id=spec.scenario_id,
        seed=spec.seed,
        condition=spec.condition.value,
        status=HD21PilotRunStatus.COMPLETED,
...
def _aggregate_ledgers(root: Path, config: HD21Config) -> dict[str, int | None]:
    summaries = []
    for spec in config.runs:
...
def run_hd21_fake_smoke(config_path: str | Path) -> dict[str, object]:
    """Exercise the exact 48-run ``run_all`` path below a temporary root."""
    pilot = HD21PilotRunner(config_path)
    with tempfile.TemporaryDirectory(prefix="evicon-hd21-v1-fake-") as temporary:
        records, receipt = pilot.run_all(
            provider_factory=lambda _spec: HD21FakeProvider(),
            root=Path(temporary) / "pilot",
            model_name="hd21-v2_1-fake",
            resume=False,
            write_receipt=False,
            network="disabled",
        )
...
def _blocked_summary(code: str) -> HD21PilotExecutionSummary:
    return HD21PilotExecutionSummary(
        status="blocked",
        error_code=code,
        network="disabled",
        results_written=False,
...
def _confirmation_error(
    *,
    ...
    if confirm_completion_reservation_cap != 442368:
        return "confirm_completion_reservation_cap_must_equal_442368"
    return None
...
def _summary_from_root(
    *,
    ...
    return HD21PilotExecutionSummary(
        ...
        completed_run_count=sum(state.status is HD21PilotRunStatus.COMPLETED for state in states),
        failed_run_count=sum(state.status is HD21PilotRunStatus.FAILED for state in states),
...
def execute_hd21_real_pilot(
    *,
    config_path: str | Path = HD21_CONFIG_RELATIVE,
    approval_path: str | Path = "configs/provenance_cascade/pilot/provenance_cascade_pilot_hd21_approval_template.toml",
    ...
    try:
        config_file = Path(config_path).resolve()
        approval_file = Path(approval_path).resolve()
        pilot = HD21PilotRunner(config_file)
        approval = load_approval(approval_file)
    except (HD21ConfigError, ValueError, OSError):
    ...
    if injected:
        model_name = test_model_name or "hd21-v2_1-fake"
        network = "disabled"
    else:
    ...
    if injected:
        ...
        def active_factory(spec: HD21RunSpec) -> LLMProvider:
...
def main(
    ...
    *,
    ...
    parser = argparse.ArgumentParser(description="H-E isolated H-D.2.1 strict pilot runner")
    ...
    if args.mode == "fake-smoke":
        print(json.dumps(run_hd21_fake_smoke(args.config), ensure_ascii=True, sort_keys=True))
        return 0
    summary = execute_hd21_real_pilot(
