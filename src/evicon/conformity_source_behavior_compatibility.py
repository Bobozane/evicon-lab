"""One-shot Provider compatibility gate for source-behavior protocol v2.

The default path is inert: it does not inspect the environment, construct a
Provider, touch the transport, or write a receipt.  The network-enabled path is
separate from the 12-case behavior qualification and permits one HTTP attempt.
"""
...
COMPATIBILITY_MODULE_VERSION = "conformity_source_behavior_compatibility.v2_1"
RECEIPT_VERSION = "conformity_source_behavior_compatibility_receipt.v2_1"
SCHEMA_VERSION = "conformity_source_behavior_response.v2"
MAX_TOKENS = 128
TEMPERATURE = 0.0
SEED = 20261110
MAX_RETRIES = 0
...
DEFAULT_RECEIPT = (
    "outputs/study-locks/"
    "conformity_source_behavior_compatibility.v2_1.json"
)

# Deriving the wire schema from the frozen Pydantic response model prevents the
# compatibility request from drifting away from the v2 adoption/share contract.
RESPONSE_SCHEMA: dict[str, object] = BehaviorQualificationResponse.model_json_schema(
    mode="validation"
)
...
class CompatibilitySchema(BaseModel):
    ...
    schema_name: Literal["conformity_source_behavior_response_v2"] = SCHEMA_NAME
    schema_version: Literal["conformity_source_behavior_response.v2"] = SCHEMA_VERSION
    schema_sha256: str = Field(default=RESPONSE_SCHEMA_SHA256, pattern=r"^[0-9a-f]{64}$")


class CompatibilityTokenUsage(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    ...
    def complete_or_absent(self) -> "CompatibilityTokenUsage":
...
class CompatibilitySafety(BaseModel):
    """Explicit safe-state markers; no request or response material is retained."""
    ...
    behavior_qualification_executed: Literal[False] = False
...
class CompatibilityResult(BaseModel):
    """The complete allowlist for CLI output."""
    ...
    token_usage: CompatibilityTokenUsage = Field(default_factory=CompatibilityTokenUsage)
    latency_ms: float | None = Field(default=None, ge=0.0)
    schema_contract: CompatibilitySchema = Field(
        default_factory=CompatibilitySchema,
        alias="schema",
    )
    attempt_count: int = Field(default=0, ge=0, le=1)
    safety: CompatibilitySafety = Field(default_factory=CompatibilitySafety)


class CompatibilityGenerationParameters(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    max_tokens: Literal[128] = MAX_TOKENS
    temperature: Literal[0.0] = TEMPERATURE
    seed: Literal[20261110] = SEED
...
class CompatibilityReceiptSafety(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
...
class CompatibilityReceipt(BaseModel):
    """Append-only, redacted evidence of one successful compatibility request."""

    model_config = ConfigDict(
        extra="forbid", frozen=True, populate_by_name=True, serialize_by_alias=True
    )

    receipt_version: Literal["conformity_source_behavior_compatibility_receipt.v2_1"]
    compatibility_module_version: Literal["conformity_source_behavior_compatibility.v2_1"]
    status: Literal["completed"]
    model: str
    finish_reason: Literal["stop"]
    parser_valid: Literal[True]
    http_status_class: Literal["2xx"]
    token_usage: CompatibilityTokenUsage
    ...
    schema_contract: CompatibilitySchema = Field(alias="schema")
    generation_parameters: CompatibilityGenerationParameters
    safety: CompatibilityReceiptSafety

    @model_validator(mode="after")
    def successful_usage_is_present(self) -> "CompatibilityReceipt":
        if not self.token_usage.available:
            raise ValueError("compatibility receipt requires token usage")
        return self


class _CountingTransport:
    """Count without retaining URLs, headers, payloads, or response bodies."""
    ...
    def post(
        ...
        if self.calls >= 1:
            raise RuntimeError("compatibility transport call cap exceeded")
        self.calls += 1
...
def minimal_public_context(
    model_name: str = "source-behavior-compatibility-model",
) -> BehaviorQualificationContext:
    """Build the only content used by the check; no study corpus is consulted."""

    content_ids = (
        "compatibility-public-content-a",
        "compatibility-public-content-b",
    )
    return BehaviorQualificationContext(
        case_id="source-behavior-v2-compatibility-synthetic",
        scenario_id="compatibility-synthetic-public-scenario",
        projection=SourceProjection.SOURCE_FREE,
        visible_content_ids=content_ids,
        public_summaries=(
            "A fictional community room may open thirty minutes earlier on weekdays.",
            "The same fictional scheduling proposal is repeated for public consideration.",
        ),
        public_root_assignments=(
            RootAssignment(content_id=content_ids[0], source_root_id=None),
            RootAssignment(content_id=content_ids[1], source_root_id=None),
        ),
...
def check_provider(
    ...
    *,
    context: BehaviorQualificationContext | None = None,
) -> CompatibilityResult:
    """Send one request to an injected Provider and retain only safe status."""

    context = context or minimal_public_context()
    ...
    try:
        parse_behavior_response(response.content, context)
        parser_valid = True
    except ValueError:
        parser_valid = False
    usage = CompatibilityTokenUsage(
    ...
    return CompatibilityResult(
        ...
        transport_category=None if completed else (
            "invalid_provider_response" if not parser_valid or finish_reason != "stop"
...
def run_compatibility_check(
    *,
    ...
    """Apply the one-shot policy without writing a receipt or result files."""
    ...
    try:
        ...
        return CompatibilityResult(
            status="blocked",
            transport_category="approval_invalid",
            safety=_safety(network_enabled=False),
        )
    if approval.acceptance_status != "accepted":
        return CompatibilityResult(
            status="blocked",
            transport_category="approval_not_accepted",
            safety=_safety(network_enabled=False),
        )
    ...
    try:
        ...
        if (
            ...
            or config.max_retries != MAX_RETRIES
            or config.timeout_seconds != int(TIMEOUT_SECONDS)
            or config.template_version != TEMPLATE_VERSION
            or config.response_schema_name != SCHEMA_NAME
        ):
            raise ValueError("compatibility configuration drift")
    except Exception:
        return CompatibilityResult(
            status="blocked",
            transport_category="configuration_invalid",
            safety=_safety(network_enabled=False),
        )
    ...
    try:
        ...
        return check_provider(
            provider,
            context=minimal_public_context(
                provider_config.model_name or "source-behavior-compatibility-model"
            ),
        )
    except Exception:
        return CompatibilityResult(
...
def receipt_from_result(
    ...
    *,
    ...
    config = load_config(DEFAULT_CONFIG)
    if approval.acceptance_status != "accepted":
        raise ValueError("source_behavior_approval_not_accepted")
    if (
...
def register_receipt(
    ...
    *,
    ...
    """Write one redacted receipt; existing files are never overwritten."""

    receipt = receipt_from_result(result, approval_path=approval_path)
...
def load_receipt(
    ...
    *,
    ...
    try:
        payload = json.loads(target.read_text(encoding="utf-8"), object_pairs_hook=_unique_pairs)
        receipt = CompatibilityReceipt.model_validate(payload)
    ...
    if observed != expected:
        raise ValueError("compatibility_receipt_binding_mismatch")
    return receipt
...
def execute_compatibility(
    *,
    ...
    """Execute the CLI semantics, including successful receipt registration."""

    if not allow_network:
        return run_compatibility_check(allow_network=False)
    try:
        target = _resolve_receipt_target(receipt_path)
    except ValueError:
        return CompatibilityResult(
    ...
    return result.model_copy(update={
        "safety": result.safety.model_copy(update={
            "compatibility_receipt_written": True,
        }),
    })
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the one-shot source-behavior v2 Provider compatibility gate"
    )
    parser.add_argument("--allow-network", action="store_true")
    arguments = parser.parse_args(argv)
    result = execute_compatibility(allow_network=arguments.allow_network)
