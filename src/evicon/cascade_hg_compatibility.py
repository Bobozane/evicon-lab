"""One-shot, offline-by-default Provider compatibility check for H-G."""
...
from .provenance_cascade_identifiability import HGPilotConfig, sha256_file
...
COMPATIBILITY_MODULE_VERSION = "provenance_cascade_hg_compatibility.v1"
COMPATIBILITY_TIMEOUT_SECONDS = 5.0
COMPATIBILITY_MAX_RETRIES = 0
COMPATIBILITY_MAX_TOKENS = 512
COMPATIBILITY_TEMPERATURE = 0.2
COMPATIBILITY_SEED = 20260911
COMPATIBILITY_MODEL_FALLBACK = "hg-compatibility-check-model"
...
class HGCompatibilityResult(BaseModel):
...
def _load_generation_contract(config_path: str | Path = DEFAULT_CONFIG_RELATIVE) -> HGPilotConfig:
    """Read only the public H-G config; evaluator-private files are not opened."""
    path = _rooted(config_path)
    try:
        config = HGPilotConfig.model_validate(tomllib.loads(path.read_text(encoding="utf-8")))
    except Exception as exc:
        raise ValueError("hg_compatibility_config_invalid") from exc
    protocol_path = _rooted(config.protocol_path if Path(config.protocol_path).is_absolute() else path.parent / config.protocol_path)
    if sha256_file(protocol_path) != config.protocol_sha256:
        raise ValueError("hg_compatibility_protocol_hash_mismatch")
    if (
        config.protocol_version != HG_PROTOCOL_VERSION
        or config.template_version != HG_TEMPLATE_VERSION
        or config.response_schema_name != HG_SCHEMA_NAME
        ...
        raise ValueError("hg_compatibility_contract_mismatch")
    return config
...
def build_minimal_public_context(
    *,
    ...
    view = ControllerPublicView(
        scenario_id="hg-compatibility-synthetic",
        agent_id="hg-compatibility-agent-01",
        round_id=0,
        claims=(
            ControllerClaimView(
                claim_id="hg-compatibility-claim-01",
                verification_status=VerificationStatus.UNVERIFIED,
                evidence_card_ids=("hg-compatibility-evidence-01",),
                public_summary="A synthetic notice proposes a temporary library schedule change.",
            ),
        ),
        evidence_cards=(
            EvidenceCard(
                evidence_id="hg-compatibility-evidence-01",
                claim="A synthetic public schedule lists the currently announced opening hours.",
                source="synthetic-public-record",
                supports=["hg-compatibility-claim-01"],
                introduced_round=0,
                visible_to=["hg-compatibility-agent-01"],
                reliability=0.5,
            ),
        ),
        provenance_nodes=(
            ProvenanceNode(
                node_id="hg-compatibility-node-01",
                content_id="hg-compatibility-content-01",
                scenario_id="hg-compatibility-synthetic",
                claim_id="hg-compatibility-claim-01",
                source_root_id="hg-compatibility-root-01",
                round_id=0,
        ...
        root_relations=(
            VisibleRootRelation(
                provenance_node_id="hg-compatibility-node-01",
                source_root_ids=("hg-compatibility-root-01",),
            ),
        ),
    )
...
def check_provider(provider: LLMProvider, *, context: CascadeAgentPromptContext | None = None) -> HGCompatibilityResult:
    """Render one H-G request, call the provider once, and strictly parse it."""
    context = context or build_minimal_public_context()
    request = render_cascade_agent_turn_hg(context)
    invoked = False
    try:
        invoked = True
...
def run_compatibility_check(
    *,
    ...
    if not allow_network:
        return HGCompatibilityResult(status="network_disabled")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="One-shot offline-by-default H-G Provider compatibility check")
