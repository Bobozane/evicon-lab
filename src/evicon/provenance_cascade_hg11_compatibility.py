"""One-shot, offline-by-default Provider compatibility check for H-G.1.1."""
...
from .provenance_cascade_hg11 import HG11PilotConfig, sha256_file
...
COMPATIBILITY_MODULE_VERSION = "provenance_cascade_hg11_compatibility.v1"
...
COMPATIBILITY_MODEL_FALLBACK = "hg11-compatibility-check-model"
...
class HG11CompatibilityResult(BaseModel):
...
def _load_generation_contract(config_path: str | Path = DEFAULT_CONFIG_RELATIVE) -> HG11PilotConfig:
    """Read only the public H-G.1.1 config; evaluator-private files are not opened."""
    path = _rooted(config_path)
    try:
        config = HG11PilotConfig.model_validate(tomllib.loads(path.read_text(encoding="utf-8")))
    except Exception as exc:
        raise ValueError("hg11_compatibility_config_invalid") from exc
    protocol_path = _rooted(config.protocol_path)
    if sha256_file(protocol_path) != config.protocol_sha256:
        raise ValueError("hg11_compatibility_protocol_hash_mismatch")
    if (
        config.protocol_version != HG11_PROTOCOL_VERSION
        or config.template_version != HG11_TEMPLATE_VERSION
        or config.response_schema_name != HG11_SCHEMA_NAME
        ...
        raise ValueError("hg11_compatibility_contract_mismatch")
    return config
...
def build_minimal_public_context(
    *,
    ...
    view = ControllerPublicView(
        scenario_id="hg11-compatibility-synthetic",
        agent_id="hg11-compatibility-agent-01",
        round_id=0,
        claims=(
            ControllerClaimView(
                claim_id="hg11-compatibility-claim-01",
                verification_status=VerificationStatus.UNVERIFIED,
                evidence_card_ids=("hg11-compatibility-evidence-01",),
                public_summary="A synthetic notice proposes a temporary library schedule change.",
            ),
        ),
        evidence_cards=(
            EvidenceCard(
                evidence_id="hg11-compatibility-evidence-01",
                claim="A synthetic public schedule lists the currently announced opening hours.",
                source="synthetic-public-record",
                supports=["hg11-compatibility-claim-01"],
                introduced_round=0,
                visible_to=["hg11-compatibility-agent-01"],
                reliability=0.5,
            ),
        ),
        provenance_nodes=(
            ProvenanceNode(
                node_id="hg11-compatibility-node-01",
                content_id="hg11-compatibility-content-01",
                scenario_id="hg11-compatibility-synthetic",
                claim_id="hg11-compatibility-claim-01",
                source_root_id="hg11-compatibility-root-01",
                round_id=0,
        ...
        root_relations=(
            VisibleRootRelation(
                provenance_node_id="hg11-compatibility-node-01",
                source_root_ids=("hg11-compatibility-root-01",),
            ),
        ),
    )
...
def check_provider(provider: LLMProvider, *, context: CascadeAgentPromptContext | None = None) -> HG11CompatibilityResult:
    """Render one H-G.1.1 request, call the provider once, and strictly parse it."""
    context = context or build_minimal_public_context()
    request = render_cascade_agent_turn_hg11(context)
    invoked = False
    try:
        invoked = True
        ...
        return HG11CompatibilityResult(
...
def run_compatibility_check(
    *,
    ...
    if not allow_network:
        return HG11CompatibilityResult(status="network_disabled")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="One-shot offline-by-default H-G.1.1 Provider compatibility check")
