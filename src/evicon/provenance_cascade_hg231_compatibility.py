...
from .cascade_agent_protocol_hg231 import (
    HG231_RESPONSE_JSON_SCHEMA, HG231_SCHEMA_NAME, parse_hg23_response,
    render_hg231_turn,
)
...
from .provenance_cascade_hg2_compatibility import (
    HG2CompatibilityResult as HG231CompatibilityResult,
...
from .provenance_cascade_hg231 import (
    DEFAULT_APPROVAL, DEFAULT_CONFIG, HG231Approval, load_config,
)

COMPATIBILITY_MODULE_VERSION = "provenance_cascade_hg231_compatibility.v1"
...
def build_minimal_hg231_context(*, model_name: str = "hg231-compatibility-model") -> HG2PromptContext:
...
def check_provider(provider: LLMProvider, *, context: HG2PromptContext | None = None) -> HG231CompatibilityResult:
    context = context or build_minimal_hg231_context()
    request = render_hg231_turn(context)
    invoked = False
    try:
        invoked = True
        ...
        return HG231CompatibilityResult(
            ...
            model=request.model_name, schema_name=HG231_SCHEMA_NAME,
        ...
        return HG231CompatibilityResult(
            ...
            schema_name=HG231_SCHEMA_NAME, attempt_count=_attempt_count(provider, invoked),
            network="enabled",
        )
...
def run_compatibility_check(
    *,
...
) -> HG231CompatibilityResult:
    if not allow_network:
        return HG231CompatibilityResult(status="network_disabled", schema_name=HG231_SCHEMA_NAME)
    config, _ = load_config(config_path)
    try:
        approval = HG231Approval.model_validate(tomllib.loads(_path(approval_path).read_text(encoding="utf-8")))
    except Exception:
        return HG231CompatibilityResult(status="blocked", provider_error_code="exact_hash_approval_invalid", schema_name=HG231_SCHEMA_NAME)
    if approval.acceptance_status != "accepted":
        return HG231CompatibilityResult(status="blocked", provider_error_code="exact_hash_human_approval_required", schema_name=HG231_SCHEMA_NAME)
    raw = ProviderConfig.from_env(allow_network=True, environment=environment)
    if raw.model_name != REQUIRED_MODEL_NAME:
        return HG231CompatibilityResult(status="blocked", provider_error_code="provider_model_mismatch", schema_name=HG231_SCHEMA_NAME)
    provider_config = raw.model_copy(update={
        ...
        "response_schema_name": HG231_SCHEMA_NAME,
        "response_schema": HG231_RESPONSE_JSON_SCHEMA,
    })
    ...
    result = check_provider(provider, context=build_minimal_hg231_context(model_name=REQUIRED_MODEL_NAME))
...
def run_fake_smoke() -> dict[str, object]:
    ...
    return {
        ...
        "response_format": "json_schema", "schema_name": HG231_SCHEMA_NAME,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    receipt_path = _path("outputs/study-locks/provenance_cascade_hg231_compatibility_receipt.json")
    if args.fake_smoke:
        ...
        if result.status == "completed":
            from .provenance_cascade_hg231_compatibility_receipt import register_compatibility_result
            try:
