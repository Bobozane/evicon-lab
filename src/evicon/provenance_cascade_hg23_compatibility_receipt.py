"""Safe one-shot compatibility receipt registration for H-G.2.3."""
...
from .cascade_agent_protocol_hg23 import HG23_SCHEMA_NAME, HG23_TEMPLATE_VERSION
from .provenance_cascade_hg2 import sha256_file
from .provenance_cascade_hg23 import (
    DEFAULT_APPROVAL, DEFAULT_CONFIG, DEFAULT_RECEIPT, HG23Approval,
    load_hg23_config,
)
from .provenance_cascade_hg23_compatibility import (
    ...
    HG23CompatibilityResult, REQUIRED_MODEL_NAME,
)

_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_COMPATIBILITY_RECEIPT = "outputs/study-locks/provenance_cascade_hg23_compatibility_receipt.json"


class HG23CompatibilityReceiptError(ValueError):
...
class HG23CompatibilityReceipt(BaseModel):
    ...
    schema_name: Literal["cascade_agent_epistemic_adoption_sharing_v1"]
    ...
    compatibility_module_version: Literal["provenance_cascade_hg23_compatibility.v1"]
    ...
    def token_sum(self) -> "HG23CompatibilityReceipt":
        if self.total_tokens != self.prompt_tokens + self.completion_tokens:
            raise ValueError("token usage mismatch")
        return self


def receipt_from_result(result: HG23CompatibilityResult) -> HG23CompatibilityReceipt:
    data = result.model_dump(mode="python")
    expected = {
        "status": "completed", "parser_valid": True, "finish_reason": "stop",
        "http_status_class": "2xx", "response_format": "json_schema",
        "schema_name": HG23_SCHEMA_NAME, "attempt_count": 1,
        "network": "enabled", "model": REQUIRED_MODEL_NAME,
    }
    if any(data.get(key) != value for key, value in expected.items()):
        raise HG23CompatibilityReceiptError("compatibility_result_not_completed")
    if any(data.get(key) is None for key in ("prompt_tokens", "completion_tokens", "total_tokens", "latency_ms")):
        raise HG23CompatibilityReceiptError("compatibility_result_incomplete")
    config, config_path, _, _, _ = load_hg23_config()
    return HG23CompatibilityReceipt(
        status="completed", parser_valid=True, finish_reason="stop",
        http_status_class="2xx", model=REQUIRED_MODEL_NAME,
        response_format="json_schema", schema_name=HG23_SCHEMA_NAME, attempt_count=1,
        ...
        amendment_receipt_sha256=sha256_file(DEFAULT_RECEIPT),
...
def register_compatibility_result(
    result: HG23CompatibilityResult,
    *, receipt_path: str | Path = DEFAULT_COMPATIBILITY_RECEIPT,
    ...
    try:
        ...
        raise HG23CompatibilityReceiptError("compatibility_receipt_path_invalid") from exc
    if target.exists():
        raise HG23CompatibilityReceiptError("compatibility_receipt_exists")
    receipt = receipt_from_result(result)
    approval_file = _path(approval_path)
    try:
        raw = approval_file.read_text(encoding="utf-8")
        approval = HG23Approval.model_validate(tomllib.loads(raw))
    except Exception as exc:
        raise HG23CompatibilityReceiptError("compatibility_approval_invalid") from exc
    if approval.acceptance_status != "accepted":
        raise HG23CompatibilityReceiptError("compatibility_approval_not_accepted")
    if (
...
def validate_receipt(path: str | Path = DEFAULT_COMPATIBILITY_RECEIPT, *, expected_hash: str | None = None) -> HG23CompatibilityReceipt:
    receipt_path = _path(path)
    if not receipt_path.is_file():
        raise HG23CompatibilityReceiptError("provider_compatibility_check_required")
    if expected_hash and sha256_file(receipt_path) != expected_hash:
        raise HG23CompatibilityReceiptError("provider_compatibility_receipt_hash_mismatch")
    try:
        receipt = HG23CompatibilityReceipt.model_validate_json(receipt_path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise HG23CompatibilityReceiptError("provider_compatibility_receipt_invalid") from exc
    config, config_path, _, _, _ = load_hg23_config()
    if (
        receipt.config_sha256 != sha256_file(config_path)
        or receipt.protocol_sha256 != config.protocol_sha256
        or receipt.amendment_receipt_sha256 != sha256_file(DEFAULT_RECEIPT)
    ):
        raise HG23CompatibilityReceiptError("provider_compatibility_receipt_binding_mismatch")
    return receipt


__all__ = [
    "HG23CompatibilityReceipt", "HG23CompatibilityReceiptError",
