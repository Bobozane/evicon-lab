...
RESUME_VERSION = "provenance_cascade_hg232_timeout_resume.v4"
...
DEFAULT_RECEIPT = "outputs/study-locks/provenance_cascade_hg232_timeout_resume_amendment_v4.json"
DEFAULT_RESUME_APPROVAL = "configs/provenance_cascade/pilot/provenance_cascade_hg232_timeout_resume_approval_v4.toml"
RESUME_HELPER_PATH = "src/evicon/provenance_cascade_hg232_resume.py"
AMENDMENT_DOCUMENT_PATH = "docs/provenance-cascade-hg232-timeout-resume-amendment-v4.md"
RESUME_HELPER_SHA256 = "00a0c78c61713db259811a75088764b1179f591b47312159f351972f4d4de238"
FAILED_LEDGER_SHA256 = "b79314ff70cddb350d14def62971a8abfce8eb68fae1c2bcf66b390a27db15bc"
FAILED_CHECKPOINT_SHA256 = "04ae613b425d1ee349ef2aa249afefbd2e30bf8b364bcef4718249810eea6e91"
FAILED_RESPONSE_AUDIT_SHA256 = "a66999aa78efb7f6a8ae7f30cbccba98440840b0b9e410c47dcca2e18001bf10"
COMPLETED_RUN_RECORD_SHA256 = {
    "hg232-cascade-hg1-false-majority-20261031-generic_dissent": "1345c571183ba74c650d00734756ea69e7ad3d502f6b5394d2723a798282f307",
    "hg232-cascade-hg1-false-majority-20261031-no_intervention": "57f7e310549ee06ffe2abd3d7d3c05f91c4f2993c6f5ccbdf1681f76795fc269",
    "hg232-cascade-hg1-false-majority-20261031-provenance_aware_controller": "d93a8c2becc07ff0d39e8044853c52179d76e0d9abf49c84b5b2fd606a7c0d30",
    "hg232-cascade-hg1-false-majority-20261031-source_blind_controller": "3fd46560e0f22e1a9ba7c37528b809249c489dc7c21fb21a3c878d5af9405310",
    "hg232-cascade-hg1-true-minority-correction-20261031-generic_dissent": "7e34ba704a785590907ae40c9073cce1c07f2fb1d7d275e84e9cfe13b0444e87",
    "hg232-cascade-hg1-true-minority-correction-20261031-no_intervention": "898e7ded1b9fa09e760fa8f41d838cfd17a71136c45ce81db159848726ed9ee6",
    "hg232-cascade-hg1-true-minority-correction-20261031-source_blind_controller": "d4d674ba175983cefb4093fa2c8719f1ea7f30cf3556a636baafc709cfca6432",
}
...
class HG232TimeoutResumeReceipt(BaseModel):
    ...
    amendment_id: Literal["provenance-cascade-hg232-timeout-resume-amendment-v4"]
    status: Literal["offline_validated_pending_network_resume_authorization"]
    resume_version: Literal["provenance_cascade_hg232_timeout_resume.v4"]
    config_sha256: str
    protocol_sha256: str
    ...
    failed_ledger_sha256: Literal[FAILED_LEDGER_SHA256]
    failed_checkpoint_sha256: Literal[FAILED_CHECKPOINT_SHA256]
    failed_response_audit_sha256: Literal[FAILED_RESPONSE_AUDIT_SHA256]
    completed_run_record_sha256: dict[str, str]
...
class HG232TimeoutResumeApproval(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    approval_id: Literal["provenance-cascade-hg232-timeout-resume-execution-v4"]
    ...
    def accepted(self) -> "HG232TimeoutResumeApproval":
        ...
        if self.acceptance_status == "accepted" and (not self.accepted_by.strip() or not all(design_flags)):
...
def _initial_audit() -> dict[str, Any]:
    ...
    artifact_hashes = {
        run_dir / "request_ledger.jsonl": FAILED_LEDGER_SHA256,
        run_dir / "agent_checkpoint.json": FAILED_CHECKPOINT_SHA256,
        run_dir / "response_audit.jsonl": FAILED_RESPONSE_AUDIT_SHA256,
    }
    if any(sha256_file(path) != expected for path, expected in artifact_hashes.items()):
        raise HG232TimeoutResumeError("timeout_resume_failed_artifact_hash_mismatch")
    for run_id, expected in COMPLETED_RUN_RECORD_SHA256.items():
        if sha256_file(_path(ROOT) / run_id / "run_record.json") != expected:
            raise HG232TimeoutResumeError("timeout_resume_completed_run_hash_mismatch")
...
def build_receipt() -> HG232TimeoutResumeReceipt:
    ...
    return HG232TimeoutResumeReceipt(
        amendment_id="provenance-cascade-hg232-timeout-resume-amendment-v4",
        ...
        failed_response_audit_sha256=FAILED_RESPONSE_AUDIT_SHA256,
        completed_run_record_sha256=COMPLETED_RUN_RECORD_SHA256,
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="H-G.2.3.2 one-shot timeout resume v4")
