"""Isolated, opt-in H-G.1 Pilot runner with offline-first gates."""
...
from .cascade_agent_protocol_hg1 import (
    ...
    CascadeAgentProtocolHG1Runtime,
...
from .provenance_cascade_hg1_compatibility_receipt import validate_receipt
...
DEFAULT_COMPATIBILITY_RECEIPT_RELATIVE = "outputs/study-locks/provenance_cascade_hg1_compatibility_receipt.json"
DEFAULT_OUTPUT_ROOT = "results/provenance-cascade-pilot-hg1-v1"


class HG1PilotRunStatus(str, Enum):
    PLANNED = "planned"
    RUNNING = "running"
    COMPLETED = "completed"
    FAILED = "failed"
...
class HG1PilotRunState(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    run_id: str
    scenario_id: str
    seed: int
    condition: str
    status: HG1PilotRunStatus = HG1PilotRunStatus.PLANNED
...
class HG1PilotBatchRecord(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    batch_version: str = "provenance_cascade_hg1_execution.v1"
    outcome_replay_contract_version: str = HG1_OUTCOME_REPLAY_VERSION
    study_id: str
    status: HG1PilotRunStatus
...
class HG1PilotReceipt(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    status: str
    study_id: str
    config_sha256: str
    ...
    approval_sha256: str
    compatibility_receipt_sha256: str
    protocol_version: str = HG1_PROTOCOL_VERSION
    template_version: str = HG1_TEMPLATE_VERSION
...
class HG1PilotExecutionSummary(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    status: str
    error_code: str | None = None
...
def _load_approval(path: Path) -> HG1Approval:
    import tomllib
    try:
        return HG1Approval.model_validate(tomllib.loads(path.read_text(encoding="utf-8")))
    except Exception as exc:
        raise CascadeRealAgentRunError("approval_invalid") from exc


class HG1PilotRunner:
    """Execute registered H-G.1 coordinates without parser recovery."""

    def __init__(self, config_path: str | Path = DEFAULT_CONFIG_RELATIVE) -> None:
        self.config_path = _rooted(config_path)
        self.config, self.scenarios, self.truths, self.schedules = load_hg1_config(self.config_path)
    ...
    def run_all(
        ...
        *,
        ...
        network: str = "enabled",
    ) -> tuple[list[CascadeRealAgentRunRecord], HG1PilotReceipt]:
        ...
        for spec in self.config.runs:
            ...
            if record_path.exists():
                ...
                batch = _replace_state(batch, _state_from_record(spec, record, run_dir), HG1PilotRunStatus.RUNNING, None)
                _atomic_model(batch_path, batch)
                continue
            if state.status is HG1PilotRunStatus.COMPLETED:
            ...
            batch = _replace_state(
                batch,
                state.model_copy(update={"status": HG1PilotRunStatus.RUNNING, "error_code": None}),
                HG1PilotRunStatus.RUNNING,
                None,
            )
            _atomic_model(batch_path, batch)
            try:
                ...
                failed = state.model_copy(update={"status": HG1PilotRunStatus.FAILED, "error_code": "provider_construction_failed"})
                batch = _replace_state(batch, failed, HG1PilotRunStatus.FAILED, "provider_construction_failed")
                _atomic_model(batch_path, batch)
            ...
            try:
                ...
                failed = state.model_copy(update={"status": HG1PilotRunStatus.FAILED, "error_code": exc.code})
                batch = _replace_state(batch, failed, HG1PilotRunStatus.FAILED, exc.code)
                _atomic_model(batch_path, batch)
                raise
            ...
            batch = _replace_state(batch, _state_from_record(spec, safe_record, run_dir), HG1PilotRunStatus.RUNNING, None)
            _atomic_model(batch_path, batch)
        replay_passed = sum(record.replay is not None and record.replay.status.value == "passed" for record in records)
        if len(records) != 48 or replay_passed != 48 or any(item.status is not HG1PilotRunStatus.COMPLETED for item in batch.runs):
            raise CascadeRealAgentRunError("batch_replay_incomplete")
        aggregates = _aggregate_ledgers(root_path, self.config)
        receipt = HG1PilotReceipt(
            status="completed",
        ...
        if write_receipt:
            if receipt_path.exists():
                ...
                existing = HG1PilotReceipt.model_validate_json(receipt_path.read_text(encoding="utf-8"))
        ...
        batch = batch.model_copy(update={"status": HG1PilotRunStatus.COMPLETED, "failure_code": None})
        _atomic_model(batch_path, batch)
...
def _initial_batch(
    ...
    *,
    ...
    return HG1PilotBatchRecord(
        study_id=config.study_id,
        status=HG1PilotRunStatus.RUNNING,
...
def _load_batch(path: Path) -> HG1PilotBatchRecord:
    if not path.is_file():
        raise CascadeRealAgentRunError("resume_batch_record_missing")
    try:
        return HG1PilotBatchRecord.model_validate_json(path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise CascadeRealAgentRunError("batch_record_invalid") from exc


def _validate_batch(batch: HG1PilotBatchRecord, config: HG1PilotConfig, config_path: Path, binding: str, root: Path, model: str) -> None:
...
def _replace_state(batch: HG1PilotBatchRecord, state: HG1PilotRunState, status: HG1PilotRunStatus, code: str | None) -> HG1PilotBatchRecord:
...
def _load_run_record(path: Path, spec: HG1RunSpec, scenario: Any) -> CascadeRealAgentRunRecord:
    try:
...
def _state_from_record(spec: HG1RunSpec, record: CascadeRealAgentRunRecord, run_dir: Path) -> HG1PilotRunState:
    ...
    if replay is None:
        raise CascadeRealAgentRunError("run_replay_incomplete")
    return HG1PilotRunState(
        run_id=spec.run_id,
        scenario_id=spec.scenario_id,
        seed=spec.seed,
        condition=spec.condition.value,
        status=HG1PilotRunStatus.COMPLETED,
...
def _aggregate_ledgers(root: Path, config: HG1PilotConfig) -> dict[str, int | None]:
...
def run_hg1_pilot_fake_smoke(config_path: str | Path = DEFAULT_CONFIG_RELATIVE) -> dict[str, object]:
    runner = HG1PilotRunner(config_path)
    with tempfile.TemporaryDirectory(prefix="evicon-hg1-pilot-fake-") as temporary:
        records, receipt = runner.run_all(
            provider_factory=lambda _spec: HG1FakeProvider(),
            root=Path(temporary) / "pilot",
            model_name="hg1-identifiability-fake",
...
def _blocked(code: str) -> HG1PilotExecutionSummary:
    return HG1PilotExecutionSummary(
        status="blocked",
        error_code=code,
        network="disabled",
...
def _partial_failure_summary(
    *,
    ...
    try:
        ...
        completed_run_count = sum(item.status is HG1PilotRunStatus.COMPLETED for item in batch.runs)
        failed_run_count = sum(item.status is HG1PilotRunStatus.FAILED for item in batch.runs)
        ...
        replay_statuses = {
            ...
            if item.status in {HG1PilotRunStatus.COMPLETED, HG1PilotRunStatus.FAILED}
        }
...
def final_preflight_hg1(
    config_path: str | Path = DEFAULT_CONFIG_RELATIVE,
    approval_path: str | Path = DEFAULT_APPROVAL_RELATIVE,
    *,
    resume: bool = False,
) -> dict[str, object]:
    """Validate all locked H-G.1 inputs without reading environment or constructing a Provider."""
    reasons: list[str] = []
    try:
        config, scenarios, _, _ = load_hg1_config(config_path)
    except Exception as exc:
        return {
            "status": "blocked",
            "ready_for_real_pilot": False,
    ...
    approval: HG1Approval | None = None
...
def execute_hg1_real_pilot(
    *,
    ...
    gate = final_preflight_hg1(config_path, approval_path, resume=resume)
    ...
    runner = HG1PilotRunner(config_path_resolved)
    ...
    def provider_factory(spec: HG1RunSpec) -> LLMProvider:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="H-G.1 public-content-identifiable provenance-cascade Pilot")
    ...
    if args.mode == "preflight":
        payload = final_preflight_hg1(args.config, args.approval, resume=args.resume)
    elif args.mode == "fake-smoke":
        payload = run_hg1_pilot_fake_smoke(args.config)
    else:
        payload = execute_hg1_real_pilot(
