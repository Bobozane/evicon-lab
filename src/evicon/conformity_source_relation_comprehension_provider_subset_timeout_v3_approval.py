"""Hash-bound review for the provider-subset timeout amendment v3.

The provider-subset v2 attempt is an immutable parent record: exactly one
request was attempted, the transport timed out, and no compatibility receipt
was created.  This module defines a new scope that changes only the transport
timeout from 5 to 15 seconds.  A completed review is never standing network
permission; the eventual command must still explicitly enable its one request.
"""

from __future__ import annotations

import hashlib
import tomllib
from datetime import date
from pathlib import Path
from typing import Literal

from pydantic import BaseModel, ConfigDict, Field, model_validator

from .conformity_identification import sha256_file
from .conformity_source_relation_comprehension_provider_subset_v2 import (
    PROVIDER_RESPONSE_SCHEMA_SHA256,
    PROVIDER_SCHEMA_NAME,
    PROVIDER_SCHEMA_VERSION,
)


_ROOT = Path(__file__).resolve().parents[2]

DEFAULT_CONFIG = (
    "configs/provenance_cascade/identification/"
    "conformity_source_relation_comprehension_compatibility_"
    "provider_subset_timeout.v3.toml"
)
DEFAULT_APPROVAL = (
    "configs/provenance_cascade/identification/"
    "conformity_source_relation_comprehension_compatibility_"
    "provider_subset_timeout_approval.v3.toml"
)
DEFAULT_MODULE = (
    "src/evicon/"
    "conformity_source_relation_comprehension_compatibility_"
    "provider_subset_timeout_v3.py"
)
PARENT_CONFIG = (
    "configs/provenance_cascade/identification/"
    "conformity_source_relation_comprehension_compatibility_provider_subset.v2.toml"
)
PARENT_APPROVAL = (
    "configs/provenance_cascade/identification/"
    "conformity_source_relation_comprehension_compatibility_"
    "provider_subset_approval.v2.toml"
)
PARENT_MODULE = (
    "src/evicon/"
    "conformity_source_relation_comprehension_compatibility_provider_subset_v2.py"
)
PROVIDER_SCHEMA_MODULE = (
    "src/evicon/conformity_source_relation_comprehension_provider_subset_v2.py"
)
PARENT_ATTEMPT_CLAIM = (
    "outputs/study-locks/"
    "conformity_source_relation_comprehension_compatibility_provider_subset.v2.claim"
)
PARENT_RECEIPT = (
    "outputs/study-locks/"
    "conformity_source_relation_comprehension_compatibility_provider_subset.v2.json"
)

COMPATIBILITY_VERSION = (
    "conformity_source_relation_comprehension_compatibility."
    "provider_subset_timeout.v3"
)
APPROVAL_VERSION = (
    "conformity_source_relation_comprehension_compatibility_"
    "provider_subset_timeout_approval.v3"
)


class SourceRelationProviderSubsetTimeoutApprovalError(ValueError):
    """Stable, redacted errors for the v3 timeout-amendment boundary."""

    def __init__(self, code: str) -> None:
        self.code = code
        super().__init__(code)


class SourceRelationProviderSubsetTimeoutConfigV3(BaseModel):
    """Immutable scope of the separately versioned timeout amendment."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    compatibility_id: Literal[
        "evicon-source-relation-comprehension-compatibility-"
        "provider-subset-timeout-v3"
    ]
    compatibility_version: Literal[COMPATIBILITY_VERSION]
    status: Literal["awaiting_explicit_authorization"]
    development_only: Literal[True]
    not_paper_result: Literal[True]
    no_causal_conclusion: Literal[True]
    network_enabled_by_default: Literal[False]
    explicit_allow_network_required: Literal[True]
    one_shot_http_request_cap: Literal[1]
    receipt_write_permitted_on_completed_check: Literal[True]

    timeout_only_amendment: Literal[True]
    provider_schema_changed: Literal[False]
    canonical_parser_changed: Literal[False]
    prompt_semantics_changed: Literal[False]
    public_material_changed: Literal[False]
    generation_parameters_other_than_timeout_changed: Literal[False]

    provider_schema_name: Literal[PROVIDER_SCHEMA_NAME]
    provider_schema_version: Literal[PROVIDER_SCHEMA_VERSION]
    provider_schema_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    provider_schema_module_path: Literal[PROVIDER_SCHEMA_MODULE]
    provider_schema_module_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")

    compatibility_module_path: Literal[DEFAULT_MODULE]
    compatibility_module_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")

    parent_compatibility_config_path: Literal[PARENT_CONFIG]
    parent_compatibility_config_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    parent_compatibility_approval_path: Literal[PARENT_APPROVAL]
    parent_compatibility_approval_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    parent_compatibility_module_path: Literal[PARENT_MODULE]
    parent_compatibility_module_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    parent_attempt_claim_path: Literal[PARENT_ATTEMPT_CLAIM]
    parent_attempt_claim_sha256: Literal[
        "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
    ]
    parent_attempt_claim_empty: Literal[True]
    parent_attempt_status: Literal["provider_error"]
    parent_failure_category: Literal["timeout"]
    parent_attempt_count: Literal[1]
    parent_receipt_path: Literal[PARENT_RECEIPT]
    parent_receipt_created: Literal[False]

    max_tokens: Literal[128]
    temperature: Literal[0.0]
    seed: Literal[20261401]
    max_retries: Literal[0]
    parent_timeout_seconds: Literal[5.0]
    timeout_seconds: Literal[15.0]
    reasoning_effort: Literal["none"]

    @model_validator(mode="after")
    def fixed_scope(self) -> "SourceRelationProviderSubsetTimeoutConfigV3":
        if self.provider_schema_sha256 != PROVIDER_RESPONSE_SCHEMA_SHA256:
            raise ValueError("source_relation_provider_subset_timeout_schema_mismatch")
        return self


class SourceRelationProviderSubsetTimeoutApprovalV3(BaseModel):
    """Researcher review of v3; never persistent network authorization."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    approval_id: Literal[
        "evicon-source-relation-comprehension-compatibility-"
        "provider-subset-timeout-v3"
    ]
    approval_version: Literal[APPROVAL_VERSION]
    acceptance_status: Literal["pending", "accepted"]
    accepted_by: str
    accepted_on: str

    compatibility_config_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    compatibility_module_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    provider_schema_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    provider_schema_module_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    parent_compatibility_config_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    parent_compatibility_approval_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    parent_compatibility_module_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    parent_attempt_claim_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    parent_attempt_status: Literal["provider_error"]
    parent_failure_category: Literal["timeout"]
    parent_attempt_count: Literal[1]
    parent_receipt_created: Literal[False]

    confirm_timeout_only_change: bool
    confirm_timeout_5_to_15_seconds: bool
    confirm_one_shot_only: bool
    confirm_zero_retries: bool
    confirm_provider_schema_unchanged: bool
    confirm_canonical_parser_unchanged: bool
    confirm_prompt_and_public_material_unchanged: bool
    confirm_other_generation_parameters_unchanged: bool
    confirm_parent_v2_immutable: bool
    confirm_parent_timeout_and_no_receipt: bool
    confirm_no_calibration_or_behavior_execution: bool
    confirm_redacted_receipt_only: bool
    confirm_no_private_or_historical_material: bool
    confirm_explicit_command_required: bool
    confirm_not_effect_or_causal_evidence: bool
    persistent_network_authorization_granted: Literal[False]
    explicit_command_authorization_required: Literal[True]

    @model_validator(mode="after")
    def accepted_review_is_complete(
        self,
    ) -> "SourceRelationProviderSubsetTimeoutApprovalV3":
        if self.acceptance_status != "accepted":
            return self
        if not self.accepted_by.strip() or not self.accepted_on.strip():
            raise ValueError(
                "source_relation_provider_subset_timeout_approval_identity_missing"
            )
        try:
            date.fromisoformat(self.accepted_on)
        except ValueError as exc:
            raise ValueError(
                "source_relation_provider_subset_timeout_approval_date_invalid"
            ) from exc
        confirmations = (
            self.confirm_timeout_only_change,
            self.confirm_timeout_5_to_15_seconds,
            self.confirm_one_shot_only,
            self.confirm_zero_retries,
            self.confirm_provider_schema_unchanged,
            self.confirm_canonical_parser_unchanged,
            self.confirm_prompt_and_public_material_unchanged,
            self.confirm_other_generation_parameters_unchanged,
            self.confirm_parent_v2_immutable,
            self.confirm_parent_timeout_and_no_receipt,
            self.confirm_no_calibration_or_behavior_execution,
            self.confirm_redacted_receipt_only,
            self.confirm_no_private_or_historical_material,
            self.confirm_explicit_command_required,
            self.confirm_not_effect_or_causal_evidence,
        )
        if not all(confirmations):
            raise ValueError(
                "source_relation_provider_subset_timeout_approval_incomplete"
            )
        return self


def _resolve(path: str | Path) -> Path:
    candidate = Path(path)
    return candidate.resolve() if candidate.is_absolute() else (_ROOT / candidate).resolve()


def compatibility_config_sha256(path: str | Path = DEFAULT_CONFIG) -> str:
    return hashlib.sha256(_resolve(path).read_bytes()).hexdigest()


def compatibility_approval_sha256(path: str | Path = DEFAULT_APPROVAL) -> str:
    return hashlib.sha256(_resolve(path).read_bytes()).hexdigest()


def _parse_config(path: str | Path) -> SourceRelationProviderSubsetTimeoutConfigV3:
    try:
        return SourceRelationProviderSubsetTimeoutConfigV3.model_validate(
            tomllib.loads(_resolve(path).read_text(encoding="utf-8"))
        )
    except Exception as exc:
        raise SourceRelationProviderSubsetTimeoutApprovalError(
            "source_relation_provider_subset_timeout_config_invalid"
        ) from exc


def load_compatibility_config(
    path: str | Path = DEFAULT_CONFIG,
) -> SourceRelationProviderSubsetTimeoutConfigV3:
    """Load v3 and verify its implementation and immutable v2 parent."""

    config = _parse_config(path)
    try:
        claim = _resolve(config.parent_attempt_claim_path)
        receipt = _resolve(config.parent_receipt_path)
        if not claim.is_file() or claim.stat().st_size != 0:
            raise ValueError("parent claim missing or nonempty")
        if receipt.exists():
            raise ValueError("parent receipt unexpectedly exists")
        expected = {
            "provider_schema_sha256": PROVIDER_RESPONSE_SCHEMA_SHA256,
            "provider_schema_module_sha256": sha256_file(PROVIDER_SCHEMA_MODULE),
            "compatibility_module_sha256": sha256_file(DEFAULT_MODULE),
            "parent_compatibility_config_sha256": sha256_file(PARENT_CONFIG),
            "parent_compatibility_approval_sha256": sha256_file(PARENT_APPROVAL),
            "parent_compatibility_module_sha256": sha256_file(PARENT_MODULE),
            "parent_attempt_claim_sha256": sha256_file(claim),
        }
        observed = {name: getattr(config, name) for name in expected}
        if observed != expected:
            raise ValueError("binding mismatch")
    except Exception as exc:
        if isinstance(exc, SourceRelationProviderSubsetTimeoutApprovalError):
            raise
        raise SourceRelationProviderSubsetTimeoutApprovalError(
            "source_relation_provider_subset_timeout_binding_mismatch"
        ) from exc
    return config


def load_compatibility_approval(
    path: str | Path = DEFAULT_APPROVAL,
) -> SourceRelationProviderSubsetTimeoutApprovalV3:
    """Load v3 review and verify that it binds the exact amended scope."""

    try:
        approval = SourceRelationProviderSubsetTimeoutApprovalV3.model_validate(
            tomllib.loads(_resolve(path).read_text(encoding="utf-8"))
        )
    except Exception as exc:
        raise SourceRelationProviderSubsetTimeoutApprovalError(
            "source_relation_provider_subset_timeout_approval_invalid"
        ) from exc
    try:
        config = load_compatibility_config()
        expected = {
            "compatibility_config_sha256": compatibility_config_sha256(),
            "compatibility_module_sha256": config.compatibility_module_sha256,
            "provider_schema_sha256": config.provider_schema_sha256,
            "provider_schema_module_sha256": config.provider_schema_module_sha256,
            "parent_compatibility_config_sha256": (
                config.parent_compatibility_config_sha256
            ),
            "parent_compatibility_approval_sha256": (
                config.parent_compatibility_approval_sha256
            ),
            "parent_compatibility_module_sha256": (
                config.parent_compatibility_module_sha256
            ),
            "parent_attempt_claim_sha256": config.parent_attempt_claim_sha256,
            "parent_attempt_status": config.parent_attempt_status,
            "parent_failure_category": config.parent_failure_category,
            "parent_attempt_count": config.parent_attempt_count,
            "parent_receipt_created": config.parent_receipt_created,
        }
        observed = {name: getattr(approval, name) for name in expected}
        if observed != expected:
            raise ValueError("approval binding mismatch")
    except Exception as exc:
        if isinstance(exc, SourceRelationProviderSubsetTimeoutApprovalError):
            raise
        raise SourceRelationProviderSubsetTimeoutApprovalError(
            "source_relation_provider_subset_timeout_approval_binding_mismatch"
        ) from exc
    return approval


__all__ = [
    "APPROVAL_VERSION",
    "COMPATIBILITY_VERSION",
    "DEFAULT_APPROVAL",
    "DEFAULT_CONFIG",
    "DEFAULT_MODULE",
    "PARENT_APPROVAL",
    "PARENT_ATTEMPT_CLAIM",
    "PARENT_CONFIG",
    "PARENT_MODULE",
    "PARENT_RECEIPT",
    "PROVIDER_SCHEMA_MODULE",
    "SourceRelationProviderSubsetTimeoutApprovalError",
    "SourceRelationProviderSubsetTimeoutApprovalV3",
    "SourceRelationProviderSubsetTimeoutConfigV3",
    "compatibility_approval_sha256",
    "compatibility_config_sha256",
    "load_compatibility_approval",
    "load_compatibility_config",
]
