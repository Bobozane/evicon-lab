"""Safe registration of a completed H-G.1.1 one-shot compatibility result."""
...
DEFAULT_RECEIPT_RELATIVE = "outputs/study-locks/provenance_cascade_hg11_compatibility_receipt.json"


class HG11CompatibilityReceiptError(ValueError):
...
class HG11RegisteredCompatibilityReceipt(BaseModel):
    """Content-free receipt for one completed H-G.1.1 compatibility request."""
    ...
    compatibility_module_version: Literal["provenance_cascade_hg11_compatibility.v1"]
    ...
    def consistent(self) -> "HG11RegisteredCompatibilityReceipt":
...
def receipt_from_result(
    ...
    *,
    ...
    if not (
        ...
        raise HG11CompatibilityReceiptError("compatibility_result_not_completed")
    config_file = _rooted(config_path)
    protocol_file = _rooted(protocol_path)
    try:
        return HG11RegisteredCompatibilityReceipt(
            status="completed",
            parser_valid=True,
            finish_reason="stop",
        ...
        raise HG11CompatibilityReceiptError("receipt_binding_file_missing") from exc
...
def _safe_receipt_path(path: str | Path) -> Path:
    ...
    try:
        ...
        raise HG11CompatibilityReceiptError("receipt_path_outside_study_locks") from exc
    return target


def _approval_with_receipt_hash(approval_path: Path, digest: str, receipt: HG11RegisteredCompatibilityReceipt) -> str:
    try:
        raw = approval_path.read_text(encoding="utf-8")
        parsed = tomllib.loads(raw)
    except Exception as exc:
        raise HG11CompatibilityReceiptError("approval_invalid") from exc
    from .provenance_cascade_hg11 import HG11Approval
    try:
        approval = HG11Approval.model_validate(parsed)
    except Exception as exc:
        raise HG11CompatibilityReceiptError("approval_invalid") from exc
    if not approval.ready or approval.acceptance_status != "accepted":
        raise HG11CompatibilityReceiptError("approval_not_accepted")
    if approval.config_sha256 != receipt.config_sha256 or approval.protocol_sha256 != receipt.protocol_sha256:
        raise HG11CompatibilityReceiptError("approval_binding_mismatch")
    if approval.controller_sha256 != sha256_file(_rooted("src/evicon/cascade_controller_hg1.py")):
        raise HG11CompatibilityReceiptError("approval_controller_hash_mismatch")
    if approval.replay_sha256 != sha256_file(_rooted("src/evicon/cascade_hg1_replay.py")):
        raise HG11CompatibilityReceiptError("approval_replay_hash_mismatch")
    if approval.amendment_sha256 != receipt.amendment_sha256:
        raise HG11CompatibilityReceiptError("approval_amendment_hash_mismatch")
    if approval.compatibility_receipt_sha256 not in {"", digest}:
        raise HG11CompatibilityReceiptError("approval_compatibility_hash_conflict")
    pattern = r'^compatibility_receipt_sha256 = "[0-9a-f]*"$'
    if len(re.findall(pattern, raw, flags=re.MULTILINE)) != 1:
        raise HG11CompatibilityReceiptError("approval_compatibility_field_invalid")
    updated = re.sub(pattern, f'compatibility_receipt_sha256 = "{digest}"', raw, flags=re.MULTILINE)
    try:
        HG11Approval.model_validate(tomllib.loads(updated))
    except Exception as exc:
        raise HG11CompatibilityReceiptError("approval_update_invalid") from exc
    return updated
...
def register_receipt(
    *,
    ...
    protocol_path: str | Path = DEFAULT_PROTOCOL_RELATIVE,
    amendment_sha256: str,
) -> tuple[HG11RegisteredCompatibilityReceipt, str, str]:
    """Register one safe result and atomically bind its hash into approval."""
    target = _safe_receipt_path(output_path)
    if target.exists():
        raise HG11CompatibilityReceiptError("receipt_exists")
    try:
        result = HG11CompatibilityResult.model_validate_json(_rooted(result_path).read_text(encoding="utf-8"))
    except Exception as exc:
        raise HG11CompatibilityReceiptError("compatibility_result_invalid") from exc
...
def validate_receipt(
    ...
    *,
    ...
    if not receipt_path.is_file():
        raise HG11CompatibilityReceiptError("compatibility_receipt_missing")
    if expected_hash and sha256_file(receipt_path) != expected_hash:
        raise HG11CompatibilityReceiptError("compatibility_receipt_hash_mismatch")
    try:
        receipt = HG11RegisteredCompatibilityReceipt.model_validate_json(receipt_path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise HG11CompatibilityReceiptError("compatibility_receipt_invalid") from exc
    if receipt.config_sha256 != sha256_file(_rooted(config_path)):
        raise HG11CompatibilityReceiptError("compatibility_config_hash_mismatch")
    if receipt.protocol_sha256 != sha256_file(_rooted(protocol_path)) or receipt.template_sha256 != sha256_file(_rooted(protocol_path)):
        raise HG11CompatibilityReceiptError("compatibility_protocol_hash_mismatch")
    if receipt.amendment_sha256 != amendment_sha256:
        raise HG11CompatibilityReceiptError("compatibility_amendment_hash_mismatch")
    return receipt


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Register or validate a safe H-G.1.1 compatibility receipt")
    ...
    except HG11CompatibilityReceiptError as exc:
