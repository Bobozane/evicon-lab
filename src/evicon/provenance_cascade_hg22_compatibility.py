"""One-shot, offline-by-default Provider compatibility check for H-G.2.2."""
...
from .openai_provider import OpenAICompatibleProvider, OpenAICompatibleTransport, ProviderConfig, ResponseFormatMode, UrllibTransport
from .provenance_cascade_hg2_compatibility import (
    HG2CompatibilityResult as HG22CompatibilityResult,
    _CountingTransport,
    _ERROR_CATEGORY_MAP,
    _attempt_count,
...
from .provenance_cascade_hg22 import DEFAULT_APPROVAL, DEFAULT_COMPATIBILITY_RECEIPT, DEFAULT_CONFIG, HG22Approval, load_config

COMPATIBILITY_MODULE_VERSION = "provenance_cascade_hg22_compatibility.v1"
...
def check_provider(provider: LLMProvider, *, context: HG2PromptContext | None = None) -> HG22CompatibilityResult:
    ...
    try:
        ...
        return HG22CompatibilityResult(
    ...
    parsed, _ = parse_hg22_response(response.content, context)
...
def run_compatibility_check(
    *,
    ...
    if not allow_network:
        return HG22CompatibilityResult(status="network_disabled", schema_name=HG22_SCHEMA_NAME)
    config, _ = load_config(config_path)
    try:
        approval = HG22Approval.model_validate(tomllib.loads(_path(approval_path).read_text(encoding="utf-8")))
    except Exception:
        return HG22CompatibilityResult(status="blocked", provider_error_code="exact_hash_approval_invalid", schema_name=HG22_SCHEMA_NAME)
    if approval.acceptance_status != "accepted":
        return HG22CompatibilityResult(status="blocked", provider_error_code="exact_hash_human_approval_required", schema_name=HG22_SCHEMA_NAME)
    raw = ProviderConfig.from_env(allow_network=True, environment=environment)
    if raw.model_name != config.required_model_name:
        return HG22CompatibilityResult(status="blocked", provider_error_code="provider_model_mismatch", schema_name=HG22_SCHEMA_NAME)
    provider_config = raw.model_copy(update={
        ...
        "response_schema": HG22_RESPONSE_JSON_SCHEMA,
    })
    ...
    result = check_provider(provider, context=build_minimal_hg2_context(model_name=config.required_model_name))
    return result.model_copy(update={"attempt_count": wrapped.calls, "model": config.required_model_name, "schema_name": HG22_SCHEMA_NAME})
...
def run_fake_smoke() -> dict[str, object]:
    ...
    conflict = _FakeProvider({
        "epistemic_stance": "uncertain",
        "behavioral_decision": "do_not_share",
        "content_ids_used": [content_id],
        "evidence_ids_used": [],
        "share_content_id": content_id,
    })
    ...
    return {
        ...
        "valid_parser": good.parser_valid,
        "conflict_parser": rejected.parser_valid,
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="One-shot H-G.2.2 Provider compatibility check")
    ...
    if args.fake_smoke:
        ...
        if result.status == "completed":
            from .provenance_cascade_hg22_compatibility_receipt import register_compatibility_result
            try:
