"""One-shot, offline-by-default Provider compatibility check for H-G.2.3."""
...
from .cascade_agent_protocol_hg23 import (
    HG23_RESPONSE_JSON_SCHEMA, HG23_SCHEMA_NAME, parse_hg23_response,
    render_hg23_turn,
)
...
from .provenance_cascade_hg2_compatibility import (
    HG2CompatibilityResult as HG23CompatibilityResult,
...
from .provenance_cascade_hg23 import (
    DEFAULT_APPROVAL, DEFAULT_CONFIG, HG23Approval, load_hg23_config,
)

COMPATIBILITY_MODULE_VERSION = "provenance_cascade_hg23_compatibility.v1"
...
def build_minimal_hg23_context(*, model_name: str = "hg23-compatibility-model") -> HG2PromptContext:
...
def check_provider(provider: LLMProvider, *, context: HG2PromptContext | None = None) -> HG23CompatibilityResult:
    context = context or build_minimal_hg23_context()
    request = render_hg23_turn(context)
    invoked = False
    try:
        invoked = True
        ...
        return HG23CompatibilityResult(
            ...
            model=request.model_name, schema_name=HG23_SCHEMA_NAME,
        ...
        return HG23CompatibilityResult(
            ...
            schema_name=HG23_SCHEMA_NAME, attempt_count=_attempt_count(provider, invoked),
            network="enabled",
        )
...
def run_compatibility_check(
    *,
...
) -> HG23CompatibilityResult:
    if not allow_network:
        return HG23CompatibilityResult(status="network_disabled", schema_name=HG23_SCHEMA_NAME)
    config, _, _, _, _ = load_hg23_config(config_path)
    try:
        approval = HG23Approval.model_validate(tomllib.loads(_path(approval_path).read_text(encoding="utf-8")))
    except Exception:
        return HG23CompatibilityResult(status="blocked", provider_error_code="exact_hash_approval_invalid", schema_name=HG23_SCHEMA_NAME)
    if approval.acceptance_status != "accepted":
        return HG23CompatibilityResult(status="blocked", provider_error_code="exact_hash_human_approval_required", schema_name=HG23_SCHEMA_NAME)
    raw = ProviderConfig.from_env(allow_network=True, environment=environment)
    if raw.model_name != REQUIRED_MODEL_NAME:
        return HG23CompatibilityResult(status="blocked", provider_error_code="provider_model_mismatch", schema_name=HG23_SCHEMA_NAME)
    provider_config = raw.model_copy(update={
        ...
        "response_schema": HG23_RESPONSE_JSON_SCHEMA,
    })
    ...
    result = check_provider(provider, context=build_minimal_hg23_context(model_name=REQUIRED_MODEL_NAME))
    return result.model_copy(update={
        "attempt_count": wrapped.calls, "model": REQUIRED_MODEL_NAME,
        "schema_name": HG23_SCHEMA_NAME,
    })
...
def run_fake_smoke() -> dict[str, object]:
    ...
    return {
        ...
        "response_format": "json_schema", "schema_name": HG23_SCHEMA_NAME,
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="One-shot H-G.2.3 Provider compatibility check")
    parser.add_argument("--allow-network", action="store_true")
    parser.add_argument("--fake-smoke", action="store_true")
    parser.add_argument("--config", default=DEFAULT_CONFIG)
    args = parser.parse_args(argv)
    receipt_path = _path("outputs/study-locks/provenance_cascade_hg23_compatibility_receipt.json")
    if args.fake_smoke:
        ...
        if result.status == "completed":
            from .provenance_cascade_hg23_compatibility_receipt import register_compatibility_result
            try:
