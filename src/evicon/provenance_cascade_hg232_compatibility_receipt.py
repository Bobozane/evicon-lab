...
from .cascade_agent_protocol_hg232 import HG232_SCHEMA_NAME, HG232_TEMPLATE_VERSION
from .provenance_cascade_hg2 import sha256_file
from .provenance_cascade_hg232 import (
    DEFAULT_APPROVAL, DEFAULT_CONFIG, DEFAULT_AMENDMENT_RECEIPT, HG232Approval,
    load_config,
)
from .provenance_cascade_hg232_compatibility import (
    ...
    HG232CompatibilityResult, REQUIRED_MODEL_NAME,
)
...
class HG232CompatibilityReceiptError(ValueError):
...
class HG232CompatibilityReceipt(BaseModel):
    ...
    compatibility_module_version: Literal["provenance_cascade_hg232_compatibility.v1"]
    ...
    def token_sum(self) -> "HG232CompatibilityReceipt":
        if self.total_tokens != self.prompt_tokens + self.completion_tokens:
            raise ValueError("token usage mismatch")
        return self


def receipt_from_result(result: HG232CompatibilityResult) -> HG232CompatibilityReceipt:
    data = result.model_dump(mode="python")
    expected = {
        "status": "completed", "parser_valid": True, "finish_reason": "stop",
        "http_status_class": "2xx", "response_format": "json_schema",
        "schema_name": HG232_SCHEMA_NAME, "attempt_count": 1,
        "network": "enabled", "model": REQUIRED_MODEL_NAME,
    }
    if any(data.get(key) != value for key, value in expected.items()):
        raise HG232CompatibilityReceiptError("compatibility_result_not_completed")
    if any(data.get(key) is None for key in ("prompt_tokens", "completion_tokens", "total_tokens", "latency_ms")):
        raise HG232CompatibilityReceiptError("compatibility_result_incomplete")
    config, config_path = load_config()
    return HG232CompatibilityReceipt(
        status="completed", parser_valid=True, finish_reason="stop",
        http_status_class="2xx", model=REQUIRED_MODEL_NAME,
        response_format="json_schema", schema_name=HG232_SCHEMA_NAME, attempt_count=1,
...
def register_compatibility_result(
    result: HG232CompatibilityResult,
    *, receipt_path: str | Path = DEFAULT_COMPATIBILITY_RECEIPT,
    ...
    try:
        ...
        raise HG232CompatibilityReceiptError("compatibility_receipt_path_invalid") from exc
    if target.exists():
        raise HG232CompatibilityReceiptError("compatibility_receipt_exists")
    receipt = receipt_from_result(result)
    approval_file = _path(approval_path)
    try:
        raw = approval_file.read_text(encoding="utf-8")
        approval = HG232Approval.model_validate(tomllib.loads(raw))
    except Exception as exc:
        raise HG232CompatibilityReceiptError("compatibility_approval_invalid") from exc
    if approval.acceptance_status != "accepted":
        raise HG232CompatibilityReceiptError("compatibility_approval_not_accepted")
    if (
...
def validate_receipt(path: str | Path = DEFAULT_COMPATIBILITY_RECEIPT, *, expected_hash: str | None = None, allow_existing_output: bool = False) -> HG232CompatibilityReceipt:
    receipt_path = _path(path)
    if not receipt_path.is_file():
        raise HG232CompatibilityReceiptError("provider_compatibility_check_required")
    if expected_hash and sha256_file(receipt_path) != expected_hash:
        raise HG232CompatibilityReceiptError("provider_compatibility_receipt_hash_mismatch")
    try:
        receipt = HG232CompatibilityReceipt.model_validate_json(receipt_path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise HG232CompatibilityReceiptError("provider_compatibility_receipt_invalid") from exc
    config, config_path = load_config(allow_existing_output=allow_existing_output)
    if (
        ...
        raise HG232CompatibilityReceiptError("provider_compatibility_receipt_binding_mismatch")
    return receipt


__all__ = [
    "HG232CompatibilityReceipt", "HG232CompatibilityReceiptError",
