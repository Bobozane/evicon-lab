...
from evicon.provenance_cascade_hg23_compatibility import (
...
from evicon.provenance_cascade_hg23_compatibility_receipt import (
    register_compatibility_result, validate_receipt,
)

ROOT = Path(__file__).resolve().parents[1]
APPROVAL = ROOT / "configs/provenance_cascade/pilot/provenance_cascade_pilot_hg23_approval_template.toml"
ENVIRONMENT = {
...
def test_valid_schema_payload_is_one_shot_and_uses_locked_parameters() -> None:
    ...
    assert schema["name"] == "cascade_agent_epistemic_adoption_sharing_v1"
...
def test_length_or_malformed_output_is_invalid_without_retry() -> None:
...
def test_transport_failures_are_safe_and_never_retried(outcome, category, status_class) -> None:
...
def test_default_is_offline_and_does_not_read_environment(monkeypatch) -> None:
    monkeypatch.setattr(
        "evicon.provenance_cascade_hg23_compatibility.ProviderConfig.from_env",
...
def test_fake_smoke_and_safe_receipt_registration(tmp_path: Path) -> None:
    ...
    target = ROOT / "outputs/study-locks/_test_hg23_compatibility_receipt.json"
    if target.exists():
        target.unlink()
    try:
        transport = FakeTransport([TransportResponse(200, _response())])
        ...
        assert receipt.schema_name == "cascade_agent_epistemic_adoption_sharing_v1"
        assert receipt.max_tokens == 1024 and receipt.seed == 20261031
