...
from evicon.conformity_source_behavior_factorization_runner_v2 import (
    DEFAULT_CONFIG,
...
from evicon.conformity_source_behavior_factorized_protocol_v2 import (
    TEMPLATE_VERSION,
    build_cases,
    protocol_self_check,
...
def test_v2_prompt_repair_states_output_invariants_without_internal_coordinates() -> None:
    report = protocol_self_check()
    assert report["protocol_version"] == "conformity_source_behavior_factorized_protocol.v2"
    assert report["template_version"] == TEMPLATE_VERSION
    assert report["prompt_contains_internal_coordinate"] is False
    assert report["explicit_action_mapping"] is True
    assert report["explicit_content_rule"] is True

    request = render_factor_request(build_cases()[1])
    assert "strongly_reject or lean_reject requires public_action=reject" in request.system_prompt
    assert "content_ids_used must be a non-empty list" in request.system_prompt
    assert "including when reflection_only=true" in request.system_prompt
    assert '"template_version":"conformity_source_behavior_factorized_turn.v2"' in request.user_prompt
...
def test_transport_failure_does_not_reuse_previous_response_metadata(tmp_path: Path) -> None:
    class FirstValidThenTimeout:
        ...
        def post(self, url, headers, payload, timeout_seconds):
            ...
            if len(self.calls) == 1:
                return TransportResponse(200, _body())
            raise TimeoutError("sensitive-timeout")

    transport = FirstValidThenTimeout()
    result = _run(tmp_path, transport)  # type: ignore[arg-type]
    assert result.status == "failed"
    assert result.transport_category == "timeout"
    assert result.completed_case_count == 1
    assert result.finish_reason is None
    assert result.parser_valid is None
...
def test_valid_transport_runs_exactly_32_cases_and_writes_redacted_receipt(tmp_path: Path) -> None:
    ...
    for call in transport.calls:
        ...
        assert "including when reflection_only=true" in call["payload"]["messages"][0]["content"]
        assert '"template_version":"conformity_source_behavior_factorized_turn.v2"' in call["payload"]["messages"][1]["content"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (TransportResponse(200, _body(content="[]")), "factorization_pilot_parser_invalid", "2xx", 1, "top_level_type"),
        ...
        (TransportResponse(200, _body(content=_valid_content() | {"content_ids_used": ["content-initial", "content-initial"]})), "factorization_pilot_parser_invalid", "2xx", 1, "duplicate_content_id"),
        ...
        (TransportResponse(200, _body(content=_valid_content() | {"public_action": "adopt"})), "factorization_pilot_parser_invalid", "2xx", 1, "judgment_action_mismatch"),
        (TransportResponse(200, _body(content=_valid_content() | {"share_content_id": "content-initial"})), "factorization_pilot_parser_invalid", "2xx", 1, "share_content_id_unexpected"),
        (TransportResponse(200, _body(content=_valid_content() | {"share_decision": "share", "share_content_id": "not-visible"})), "factorization_pilot_parser_invalid", "2xx", 1, "share_content_id_invalid"),
    ...
    ids=["malformed-json", "top-level-type", "unknown-field", "duplicate-content", "unavailable-content", "judgment-action-mismatch", "share-content-unexpected", "share-content-invalid", "finish-length", "timeout", "connection", "401", "429", "500", "response-format-unsupported"],
)
