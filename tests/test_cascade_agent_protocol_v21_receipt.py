...
from evicon.cascade_agent_protocol_v21_receipt import (
    COMPATIBILITY_VERSION,
    HD21RegisteredCompatibilityReceiptError,
    DEFAULT_RECEIPT_RELATIVE,
    register_receipt,
    sha256_file,
    validate_receipt,
)
from evicon.cascade_agent_protocol_v21_pilot import final_preflight

ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/provenance_cascade/pilot/provenance_cascade_pilot_hd21.v1.toml"
PROTOCOL = ROOT / "src/evicon/cascade_agent_protocol_v21.py"
...
def test_registered_receipt_has_only_safe_observed_fields() -> None:
    ...
    assert receipt.compatibility_module_version == COMPATIBILITY_VERSION
    assert receipt.prompt_tokens == 401
    assert receipt.completion_tokens == 149
    assert receipt.total_tokens == 550
    assert receipt.max_tokens == 512
...
def test_receipt_registration_never_overwrites(tmp_path) -> None:
    ...
    target = ROOT / "outputs" / "study-locks" / "_test_hd21_receipt.json"
    if target.exists():
        target.unlink()
    try:
        _, digest = register_receipt(output_path=target, config_path=CONFIG, protocol_path=PROTOCOL)
        assert digest == sha256_file(target)
        with pytest.raises(HD21RegisteredCompatibilityReceiptError) as error:
