...
from evicon.provenance_cascade_hg232_calibration import (
    HG232CalibrationRunner,
    execute_real,
    final_preflight,
    run_fake_smoke,
)
...
FORMAL_ROOT = ROOT / "results/provenance-cascade-hg232-adoption-identifiability-v1"
...
def test_final_preflight_is_offline_and_waits_for_execution_authorization(monkeypatch) -> None:
    monkeypatch.setattr(
        "evicon.provenance_cascade_hg232_calibration.ProviderConfig.from_env",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("provider must not be constructed")),
    )
    report = final_preflight()
    assert report["blocking_reasons"] == []
    assert report["ready_for_network_authorization"] is True
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"allow_network": True, "confirm_run": True, "confirm_request_cap": 288, "confirm_completion_reservation_cap": 1}, "confirm_completion_reservation_cap_must_equal_589824"),
    ],
)
...
def test_model_mismatch_rejected_before_provider_construction(monkeypatch) -> None:
    monkeypatch.setattr(
        "evicon.provenance_cascade_hg232_calibration.final_preflight",
...
def test_no_overwrite_resume_and_binding_protection(tmp_path: Path) -> None:
    ...
    audit_lines = (root / runner.runs[0].run_id / "response_audit.jsonl").read_text(encoding="utf-8").splitlines()
    decision_lines = (root / runner.runs[0].run_id / "behavior_decisions.jsonl").read_text(encoding="utf-8").splitlines()
    assert len(audit_lines) == len(decision_lines) == 18
    assert "finish_reason" in audit_lines[0] and "epistemic_stance" not in audit_lines[0]
    assert "epistemic_stance" in decision_lines[0] and "finish_reason" not in decision_lines[0]
...
def test_old_failure_hashes_remain_unchanged_when_new_root_exists() -> None:
    ...
    if FORMAL_ROOT.exists():
        ...
        # The pre-resume SHA remains sealed in the v4 amendment.  The live
        # directory now contains the separately audited completed batch.
        assert sha256_file(FORMAL_ROOT / "calibration_batch_record.json") == "edc3836244ffc71f13b14443f88d79eeb0db2c1b77100777268347355047541d"
