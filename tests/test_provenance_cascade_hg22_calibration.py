...
from evicon.provenance_cascade_hg22_calibration import (
    HG22CalibrationRunner,
    execute_real,
    final_preflight,
    run_fake_smoke,
)
...
FORMAL_ROOT = ROOT / "results/provenance-cascade-hg22-eligibility-calibration-v1"
...
def test_final_preflight_is_offline_and_waits_for_execution_authorization(monkeypatch) -> None:
    monkeypatch.setattr(
        "evicon.provenance_cascade_hg22_calibration.ProviderConfig.from_env",
...
def test_model_mismatch_rejected_before_provider_construction(monkeypatch) -> None:
    monkeypatch.setattr(
        "evicon.provenance_cascade_hg22_calibration.final_preflight",
