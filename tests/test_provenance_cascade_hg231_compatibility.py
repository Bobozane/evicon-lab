...
from evicon.provenance_cascade_hg231_compatibility import (
...
from evicon.provenance_cascade_hg231_compatibility_receipt import (
    register_compatibility_result, validate_receipt,
)

ROOT = Path(__file__).resolve().parents[1]
APPROVAL = ROOT / "configs/provenance_cascade/pilot/provenance_cascade_pilot_hg231_approval_template.toml"
ENVIRONMENT = {
...
def test_valid_schema_payload_is_one_shot_and_uses_locked_parameters(tmp_path: Path) -> None:
    ...
    assert schema["name"] == "cascade_agent_epistemic_adoption_sharing_v1_provider_subset"
...
def test_default_is_offline_and_does_not_read_environment(monkeypatch) -> None:
    monkeypatch.setattr(
        "evicon.provenance_cascade_hg231_compatibility.ProviderConfig.from_env",
...
def test_fake_smoke_and_safe_receipt_registration(tmp_path: Path) -> None:
    ...
    target = ROOT / "outputs/study-locks/_test_hg231_compatibility_receipt.json"
    if target.exists():
        target.unlink()
    try:
        transport = FakeTransport([TransportResponse(200, _response())])
        ...
        assert receipt.schema_name == "cascade_agent_epistemic_adoption_sharing_v1_provider_subset"
